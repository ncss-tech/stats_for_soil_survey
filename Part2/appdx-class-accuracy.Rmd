...
## class probabilities
ss <- imap(s, .f = function(i, idx) {
  .res <- pluck(i, 'predictions.long')
  .res$example <- idx
  return(.res)
})
...
# plot style
tps <- tactile.theme(superpose.line = list(lwd = 1, alpha = 0.85))
...
p.1 <- densityplot(
  ...
  pch=NA, 
  xlim = c(-0.1, 1.1), 
  scales = list(alternating = 3, x = list(tick.number = 5)), 
...
# uncertainty metrics
tps <- tactile.theme(superpose.line = list(lwd = 2, alpha = 0.85))
...
p.2 <- densityplot(
  ...
  strip = strip.custom(bg = grey(0.85)), 
  scales = list(alternating = 3, y = list(rot = 0), x = list(tick.number = 5)), 
  xlab = '', 
  par.settings = tps, 
  panel = function(...) {
...
tps <- tactile.theme(plot.symbol=list(col='royalblue', pch=16, cex=0.85, alpha=0.25))

# how do these two compare in general?
p.3 <- xyplot(
  Shannon.H ~ CI | example, 
  data = ss, 
  auto.key = list(columns = 3, lines = FALSE, points = TRUE), 
  as.table = TRUE, 
  layout = c(3, 1),
