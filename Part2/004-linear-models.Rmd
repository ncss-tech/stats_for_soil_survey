...
data <- extract(geodata_r, mast_sf, bind = TRUE) |>
  as.data.frame()
...
# random sample
vars <- c("elev", "temp", "precip", "solarcv", "tc_1", "tc_2")
idx <- which(names(geodata_r) %in% vars)
geodata_s <- spatSample(geodata_r[[idx]], size = 1000, method = "regular")
...
geodata_l <- tidyr::pivot_longer(geodata_df, cols = -source)
...
par(mfrow = c(5, 2))
# partial residuals
lm(formula(fit_ols), data = data) |>
  termplot(partial.resid = TRUE, col.res = "black", pch = 16)
...
# Anova
final_ols |> anova()
final_ols |> anova() |> plot(what = "remaining R2")
```
...
final_ols |>
  Predict(elev = NA, solarcv = c(23, 33, 51)) |>
  ggplot() +
...
mast_r <- predict(geodata_r, final_ols, fun = predfun, index = 1:2)
