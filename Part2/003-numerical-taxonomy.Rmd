...
Simulated redoximorphic feature colors, contrast classes and CIE $\Delta{E_{00}}$. Details [here](https://ncss-tech.github.io/AQP/aqp/color-contrast.html).
...
# # "nice" colors
# cols <- hcl.colors(25, palette = 'mako')
...
#   main = 'Pair-Wise Distances', 
#   scales = list(alternating = 3), 
#   panel = function(x, y, z, ...) {
#     panel.levelplot(x, y, z, ...)
#     idx <- which(!is.na(z))
#     # panel.text(x[idx], y[idx], z[idx], font=2, col='firebrick')
 ...
 * **Key point:** Each characteristic is its own dimension in property-space.
  ...
  + Multiple dimensions are simple to define in code but [are hard to visualize](https://www.youtube.com/watch?v=UnURElCzGc0).
...
The branches of a dendrogram can be rotated like a mobile, so that the final ordering of the terminal "leaves" approximates an alternate ranking. For example, branches of the following dendrogram (right-hand side) have been rotated to approximate the expected [hydrologic gradient from summit to toeslope](https://ncss-tech.github.io/AQP/sharpshootR/geomorphic-summaries-and-ordering.html).

![](static-figures/ava-block-diagram-hillslope.png)

This figure is based on component records from SSURGO, arranged using functions from the sharpshootR package, and presented in [SoilWeb/SDE](https://casoilresource.lawr.ucdavis.edu/sde/?series=ava#sibling-summary).
...
![](static-figures/munsell-hue-dend.png)

![](static-figures/munsell-hue-mds.png)
...
```{r eval = TRUE, echo=FALSE, results='hide', fig.width=10, fig.height=5, message=FALSE}
# re-make data, this time with all profiles
data('sp4', package = 'aqp')
sp4 <- sp4[, c('name', 'clay', 'sand', 'Mg', 'Ca', 'CEC_7')]
sp4.scaled <- data.frame(name=sp4[, 1], round(scale( sp4[, -1]), 2))
...
```{r eval = TRUE, echo=FALSE, results='hide', message=FALSE, fig.width=10, fig.height=4}
# figure margins
 ...
 * [Evaluating an ordination using "stress" and the Shephard Diagram](https://ncss-tech.github.io/stats_for_soil_survey/exercises/numerical-taxonomy/nMDS-eval-01.html)


### Pair-Wise Distances Between Soil Profiles

```{r eval = TRUE, fig.width=10, fig.height=5, echo=FALSE, message=FALSE}
# init example data
data(sp4)
depths(sp4) <- id ~ top + bottom
...
par(mar = c(0, 0, 3, 2))
plotProfileDendrogram(sp4, clust, dend.y.scale = 50, scaling.factor = 0.66, y.offset = 3.5, width=0.3, cex.names = 0.7, color='ex_Ca_to_Mg', col.label='Exchageable Ca to Mg Ratio', name.style = 'center-center', max.depth = 40)
```
...
## Exercises
Use the following examples to assist with exercises 1 and 2.


  * [Exercise 1: Num. Tax. Applied to Soil Series Data](https://ncss-tech.github.io/stats_for_soil_survey/exercises/numerical-taxonomy/assignment-1.html)
  * [Exercise 2: Clustering of Image Colors](https://ncss-tech.github.io/stats_for_soil_survey/exercises/numerical-taxonomy/cluster-image-colors.html)
...
```{r eval = TRUE,fig.width=12, fig.height=7.5}
...
```{r eval = TRUE, fig.width=6.5, fig.height=4, warning = FALSE}
...
```{r eval = TRUE, results='hide', fig.width=12, fig.height=6}
...
The [Shepard diagram](https://rdrr.io/rforge/vegan/man/goodness.metaMDS.html) (stress plot) is used to assess the quality of a nMDS ordination. There should be as little scatter as possible in this diagram.
```{r eval = TRUE, results='hide', fig.width=6, fig.height=6}
stressplot(s)
```

#### nMDS of Very Large Data
...
![](static-figures/rmf-mds.png)



<!-- #### Rotations -->
<!-- Mapping from one ordination to another: [Procrustes_analysis](https://en.wikipedia.org/wiki/Procrustes_analysis). See the [color contrast tutorial for some applications](https://ncss-tech.github.io/AQP/aqp/color-contrast.html). -->

<!-- ![](static-figures/hue-position-rotation-example.png) -->
...
```{r eval = TRUE, fig.width=12.5, fig.height=5}
...
```{r eval = TRUE, fig.width=13, fig.height=6}
par(mar = c(0, 1, 1, 4))
