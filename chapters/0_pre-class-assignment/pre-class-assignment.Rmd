...
For example, download and install the {remotes} package from CRAN:
...
To install the **R** packages used in this class copy all of the code from the box below and paste into the **R** console window. 

  1. Paste after the command prompt (`>`) and press **enter**. 
  
  2. If this is the first time you've installed any packages, **R** will ask you if you want to create a local repository in your User `Documents` folder. Click **Yes**.
...
ipkCRAN <- function(p, up = TRUE){
  ...
  if (up) {
    # default is to upgrade everything
...
packages <- c(
    ...
    # gis
    "rgdal", "raster", "sp", "sf", "terra", "gdalUtils", "rgrass7", "RSAGA", "exactextractr", "fasterize",
    # data management
...
### No output is produced after pasting into console
 ...
 If you do not have a new command prompt (`>`) and a blinking cursor on the left hand side of your console after you run a command, **R** may think you are still in the middle of submitting multiline input to the "read-eval-print-loop" (REPL). If this is not expected you are possibly missing closing quotes, braces, brackets or parentheses. **R** needs to know you were done with your expression, so you may need to supply some input to get the command to be complete, or at least "error out" so you can try again. 
 
You can manually paste code line-by-line -- which is useful but prone to above mentioned input errors for multiline expressions. Alternately, you can run commands or an entire file using the GUI or keyboard shortcuts such as **Ctrl+Enter**. You have a chance to try this in the example at the end.
...
To install the latest version of packages from the Algorithms for Quantitative Pedology (AQP) suite off GitHub using the `remotes` package. These packages tend to be updated more frequently on GitHub than they are on CRAN.
