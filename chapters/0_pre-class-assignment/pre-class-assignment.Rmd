...
These packages aren't available to our machines as we are stuck at an older version of R, or not currently on CRAN. The following code blocks should be run depending on the version of R you have installed.

The technique we use to fill in gaps in binary package availability is to use the [Microsoft R Archive Network Time Machine](https://mran.microsoft.com/timemachine).
...
```{r, eval=FALSE}
# for R 4.0.0 ONLY
install.packages("https://cran.microsoft.com/snapshot/2019-04-15/bin/windows/contrib/3.5/Rcpp_1.0.1.zip", repos = NULL)
```

Next, install the latest version of packages from the AQP suite. Windows 10 users will need the latest version of soilDB in order to connect to the local NASIS database.
