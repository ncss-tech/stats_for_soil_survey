...
d <- diagnostic_hz(pedons2)
idx <- unique(d[d$diag_kind == "argillic horizon" & d$featdept < 50, "peiid"])
pedons2$argillic.horizon <- site(pedons2)$peiid %in% idx
...
data_m <- melt(s[c("argillic", "ld", "elev_field", "bedrckdepth", "pavement")], id = "argillic.horizon")
bwplot(argillic ~ wetness, data = data, scales = list(x = "free"))
