...
R is a free, open-source software and programming language developed in 1995 at the University of Auckland. R is capable of executing simple and complex mathematical, statistical, and graphical functions [(Ikaha and Gentleman, 1996)](https://www.stat.auckland.ac.nz/~ihaka/downloads/R-paper.pdf).  It is a dialect of the S language and is case sensitive. The R interface allows you to execute functions using a command prompt (>). Amongst soil scientists and ecologists, R has become popular for exploring data, modeling soil and plant relationships, and generating raster-based model predictions.  
...
- R has a built-in editor to edit and select code to run. Some people, however, find it easier to use Notepad. 
...
To run a command in R, you type the command after the command prompt `>` in the R Console window and press the **Enter** key. You can edit command text in R Editor, and you can also highlight a command, right-click, and select **Run.** 
...
First, create an example file by copying the contents below. Paste the content into Notepad and save the file as sand_example.txt in the C:\workspace folder.  
...
The `ls()` and `rm()` functions are also available in RGui in the Misc menu. 

![R GUI image](figure/ch1_fig4_rgui.jpg)  

## 1.3  Saving R Files  

In R, you can save several types of files to keep track of the work you do. The file types include workspace, script, history, and graphics. It is important to save often because R, like other software, may crash periodically. Such problems are especially likely when working with large files. You can save your work in R via the command line or the File menu.  
...
You may want to save your scripts (whether they were written in R Editor or an ancillary program such as Notepad) so that you can reference them in the future, edit them as needed, and keep track of what you've done. To save R scripts in the RGui, make sure the R Editor window is active and click the **File** menu and then **Save as.** Save scripts with the .R extension. R assumes that script files are saved with only that extension. If you are using another text editor, you won't need to worry about saving your scripts in R. You can always copy and paste them in to the R Console or R Editor from your text editor.  

![R GUI image](figure/ch1_fig13_rgui.jpg)  

To open an R script, click **File** > **Open script.**  
...
When the R Console window is active in RGui, you can click the **Packages** drop-down menu. (Click on the R Console window if it is not currently active.) You will see options to set a CRAN mirror and to load, install, and update packages. You can select more than one package to install at a time by holding down the Ctrl key. The Comprehensive R Archive Network (CRAN) is a collection of sites that carry identical material for R. Select the location closest to you.    
...
Prior to installing a package, you need to select a CRAN mirror site to download from. Select the site closest to your physical location.
...
The "regions" option specifies a character vector that names the polygons to draw. Each database is composed of a collection of polygons, and each polygon has a unique name. When a region is composed of more than one polygon, the individual polygons have the name of the region, followed by a colon and a qualifier, as in michigan:north and michigan:south. Each element of a region is matched against the polygon names in the database and, according to exact matching, a subset is selected for drawing. The default selects all polygons in the database.  
...
In the **Enter name for data set** box, type sand2. Make sure that **Field Separator** is set to **Commas.** 

![R GUI image](figure/ch1_fig21_rgui.jpg)  

Once you click **OK,** a Windows Explorer box appears. Select the file you wish to import. For this example, you want to select sand.txt. Next, click **View data set** in the R Commander GUI functions and confirm that the data imported with five columns.  

![R GUI image](figure/ch1_fig22_rgui.jpg)  

A quicker way to import data that is already imported as an object in R is to click on the box to the right of **Data set** in the R Commander GUI. This opens a window that lists all of the dataset objects that have been created during your R session. In our case, we want to select the **sand2** data set. 

![R GUI image](figure/ch1_fig5_rgui.jpg)

### 1.5.2 Creating Graphs 

In the **Graphs** drop-down menu in the Rcmdr GUI, click **Histogram.**  

![R GUI image](figure/ch1_fig23_rgui.jpg)

Select **sand** as the variable, click  the **Option** tab, and make sure the number of bins is set to **auto** for the first time. You can edit it later. Click **OK.** A plot appears in a separate window within the RGui. 

```{r, echo=FALSE, warning=FALSE}
hist(sand$sand, col="darkgrey", xlab="sand", ylab="frequency", main="")
```

Navigate back to the R Commander window and select the **Graphs** menu again. This time, click **Boxplot** and choose **sand** as the variable. Click **Plot by groups** and select **landuse** as the variable. Next, switch to the **Options** tab to ensure that outliers are identified **automatically.** Click **OK.** 

![R GUI image](figure/ch1_fig25_rgui.jpg)  

If you want to change something slightly, edit the command line in the script window, select the entire command, and click **Submit.** Note that commands in the R Commander script window are not preceded by an angle bracket (>) as they are in the R Console.  
...
You can use the options in the Statistics menu to calculate basic summary statistics. Examples are provided below.  
...
Just as in the R editor window, you can save the Rcmdr script that you generated. In the **File** drop-down menu in the R Commander GUI, click **Save script as.**
...
You can open a saved R commander script in R commander or in R editor. Sometimes, however, scripts saved with R Commander include code for R commander that isn't apparent and won't run directly from R editor. As an example, open the script we just saved. First, open the script with R commander, and then open it in R editor. Using the saved R commander script in R Commander, open and view R Commander and then navigate to **File** > **Open Script file** > **sand_rcmdr.R.** You will be asked if you want to save the current log file (Click **No** to clear without saving). Your saved script will appear in the Script Window.  
...
Click on the RGui console, navigate to the **File** drop-down menu, and click **Open script.** A Windows Explorer box appears. Navigate to sand_rcmdr.R. A new R editor window opens up within R. Select the first two lines of the script using your mouse, right-click, and click **Run line or selection.**
...
Note how changing the breaks argument alters the appearance of the graph. The breaks argument specifies how the individual values should be counted in bins or groups. The xlim argument specifies where to set the upper and lower limit of the x-axis.  
...
The [soilDB](https://r-forge.r-project.org/scm/viewvc.php/*checkout*/docs/soilDB/soilDB-Intro.html?root=aqp) package for R was created to simplify analysis of data from USDA-NCSS soil databases. It includes a series of convenience functions that allow you to quickly access soil data from a wide range of sources, including NASIS, KSSL, OSDs, and several others. The convenience functions typically return data as a [`SoilProfileCollection` object](https://r-forge.r-project.org/scm/viewvc.php/*checkout*/docs/aqp/aqp-intro.html?root=aqp), which is a special datatype designed specifically for soil information. In this module, we are going to focus on the `fetchKSSL()` function. Use the following script to examine KSSL data for the Hartleton soil series. Feel free to replace Hartleton with a soil series in your MLRA. 
...
See the related [tutorial on fetchKSSL()](https://r-forge.r-project.org/scm/viewvc.php/*checkout*/docs/soilDB/KSSL-demo.html?root=aqp) for more details. 
