...
The optimal CP value is 0.030865 for spodintmodel and 0.050459 for spodintmodel2. Since both spodic intensity models overfit that data, they will need to be pruned using the `prune()` function.

```{r, fig.width=7, fig.height=8}
pruned<-prune(spodintmodel, cp=0.030865)
printcp(pruned)
plot(pruned)
text(pruned, cex=0.8)
...
The misclassification rate (in cross-validation) for the spodintmodel was 59% (root node error * xerror * 100) which dropped to 33% in the spodintmodel2. Why did the performance of these models differ significantly?
...
conf.matrix <- table(test$spodint, predict(pruned,type="class"))
rownames(conf.matrix) <- paste("Actual", rownames(conf.matrix), sep = ":")
colnames(conf.matrix) <- paste("Pred", colnames(conf.matrix), sep = ":")
print(conf.matrix)

conf.matrix2 <- table(test$newcolumn, predict(pruned2,type="class"))
