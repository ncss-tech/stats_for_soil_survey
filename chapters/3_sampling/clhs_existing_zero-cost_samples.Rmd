...
# overplot with yellow to show which ones were the 30 that we started with 
points(obs.existing, bg = 'orange', pch=21)
points(obs.cheap, bg = 'purple', pch=21)
...
This would suggest, as expected, that additional bias is introduced to cLHS by additional constraints on Ordinary cLHS method. The degree of bias in the existing data you force-include in cost-constrained cLHS will impact the deviation from the ideal. These differences may be essentially numerical noise -- given the precision/accuracy of input raster data -- and future work will try to restate differences in cLHS and determin if there is a better way to make use of the information in prior data.