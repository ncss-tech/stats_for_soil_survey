...
From the figure above, you can see how difficult it is to separate two overlapping populations (e.g., horizons). Also the number of samples required doubles with a 1 unit increase in the standard deviation. This simplistic example, although enlightening, is hard to implement in practice and for our purposes only serves as a theoretical exercise. 
...
To gauge how representative a sampling strategy is we can compare it to other variables we think might coincide with the soil properties or classes we're interested in, such as slope gradient, slope aspect and vegetative cover. These other variables may be used to stratify the sampling design or assess the representativeness of our existing samples (e.g. NASIS pedons). In addition, if the samples are going to be used for spatial interpolation we could assess the geometry of the spatial coordinates using spatial metrics (Hengl, 2009), which has special significance when determining the spatial resolution of the predictions (Hengl, 2006).
...
The back-to-back histogram figures above illustrate the differences between a large and small set of samples using several sampling designs. Clearly the cLHS approach does the best job duplicating the distribution of elevation, because elevation is explicitly used in the stratification process. The comparison is less severe when simply examining the summary metrics, but again cLHS more closely resembles the larger sample. Other comparisons are possible, via the approaches demonstrated in the following chapters.
...
**Purpose**
Investigators in the Monongahela National Forest were interested in quantifying the depth of organic surface horizons in soils correlated to the Mandy soil series  that formed under red spruce canopy on backslopes in the Upper Greenbrier Watershed (HUC 8 -05050003).  
