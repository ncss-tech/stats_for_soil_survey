...
head(s_m)
bwplot(argillic.horizon2 ~ value | variable, data = s_m, scales = list(relation = "free"))
```

Looking at our continuous variables we don't appear to have much if any separation between the presence/absence of argillic horizons. 
...
Examining the above frequency table we can see that argillic horizons occur predominantly on fan remnants as was alluded too earlier. However they also seem to occur frequently on many other landforms. Other observations seem to have made use of curious landform combinations or redundant terms.
...
# By proportion
round(prop.table(table(s$describer2, s$argillic.horizon2), margin = 1) * 100)
```
...
# writeOGR(pedons_sp, dsn = "F:/geodata/project_data/8VIC", "pedon_locations", driver = "ESRI Shapefile") # write shapefile of pedons
```

Notice in the above figure the number of points that fall out the survey boundary. What it doesn't show in the points in the Ocean or Mexico.
...
files <- c(
  ...
  cluster = "cluster15.tif"
  )
geodata <- sapply(files, function(x) paste0(folder, x)) 
geodata <- stack(geodata)

data <- data.frame(
   as.data.frame(pedons_sp)[c("pedon_id", "taxonname", "argillic.horizon2", "x_std", "y_std", "describer2")],
   extract(geodata, pedons_sp)
   )
