...
```{r spatial}
data$argillic2 <- data$argillic == TRUE & data$mrvbf > 0.15 # Include only argillic horizons that only occur on fans. Argillic horizons that occur on hills and mountains more than likely form by different process, and therefore would require a different model.

data_m <- subset(data, select = - c(pedon_id, taxonname, x_std, y_std, describer2))
data_m <- melt(data_m, id = "argillic2")
bwplot(argillic2 ~ value | variable, data = data_m, scales = list(x = "free"))

# Argillic horizons seem to occur over a limited range of twi. So lets rescale twi by substracting its median
aggregate(twi ~ argillic2, data = data, summary)
...
```{r fitting}
test <- glm(argillic2 ~ ., data = data, family = binomial(link = "cloglog"))
summary(test)
confusionMatrix(test$fitted.values > 0.5, as.logical(test$y), positive = "TRUE")

test1 <- glm(argillic2 ~ 1, data = data, family = binomial(link = "cloglog"))

test2 <- glm(argillic2 ~ twi_sc + slope + z2str + ls_1, data = data, family = binomial(link = "cloglog"))
summary(test2)
```
...
```{r evaluation}
comp <- cbind(data2[c("argillic2", "cluster")], pred = predict(test2, data2, type = "response") > 0.5)
comp <- na.exclude(comp)
confusionMatrix(comp$pred, comp$argillic2, positive = "TRUE")

comp2 <- comp[comp$argillic2 == TRUE, ]
group_by(comp2, cluster) %>% summarize(sum(argillic2), sum(pred), recall = round(sum(pred == argillic2) / length(argillic2), 2))


data3 <- subset(data2, !(cluster %in% c(2, 6, 7, 8, 13, 15)))
aggregate(twi ~ argillic2, data = data2, summary)
data3$twi_sc2 <- abs(data3$twi - 13.8)

test <- glm(argillic2 ~ ., data = data3, family = binomial(link = "cloglog"))

test2 <- glm(argillic2 ~ slope + twi_sc2 + ls_1 + mrvbf + z2str, data = data3, family = binomial(link = "cloglog"))
summary(test2)
comp <- cbind(data3[c("argillic2", "cluster")], pred = predict(test2, data3, type = "response") > 0.5)
comp <- na.exclude(comp)
confusionMatrix(comp$pred, comp$argillic2, positive = "TRUE")
comp2 <- comp[comp$argillic2 == TRUE, ]
group_by(comp2, cluster) %>% summarize(sum(argillic2), sum(pred), recall = round(sum(pred == argillic2) / length(argillic2), 2))
...
writeRaster(r, "test.tif", overwrite = T, progress = "text")
```
