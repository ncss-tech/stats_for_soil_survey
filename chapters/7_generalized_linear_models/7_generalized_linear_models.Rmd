...
table(s_sub$landform.string, s_sub$argillic.horizon) # cross tabulate landform vs taxonname
...
data_m <- subset(data, select = - c(peiid, taxonname, x_std, y_std, describer2))
...
aggregate(data[c("twi", "z2str")], list(data$jumborox), median)
data$twi_sc <- abs(data$twi - 13.4)
data$z2str_sc <- abs(data$z2str - 15.3)

data2 <- subset(data, select = - c(pedon_id, describer2, argillic.horizon, taxonname))

test <- glm(argillic.horizon ~ ., data = data2, family = binomial(link = "cloglog"))
summary(test)
confusionMatrix(test$fitted.values > 0.5, as.logical(test$y), positive = "TRUE")

test2 <- glm(argillic.horizon ~ 1, data = data2, family = binomial(link = "cloglog"))
summary(test2)
