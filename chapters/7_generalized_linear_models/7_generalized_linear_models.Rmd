...
# Fit full model
full <- glm(argillic ~ z + log(slp) + sw + twi + twi_sc + log(ch + 1) + log(z2str + 1) + mrrtf + log(mrvbf + 1) + solar + precip + precipsum + temp + mast + ls_1 + ls_2 + ls_3 + ls_4 + ls_5 + ls_6 + pc_1 + pc_2 + pc_3 + pc_4 + pc_6 + tc_1 + tc_2 + tc_3 + gsi + ndvi + k + th + u, data = train[complete.cases(train), idx], family = binomial) # "~ ." includes all columns in the data set
...
```{r final}
argi.glm <- glm(argillic ~ twi_sc + log(slp) + tc_1 + tc_2 + cluster2, data = train, family = binomial)
```
...
train$predict <- train$predict > 0.2
...
train_sub <- subset(train, ! cluster2 %in% c(12, 13, 2, 6, 7))
...
sub.glm <- glm(argillic ~ log(slp) + twi_sc + tc_1 + tc_2, data = train_sub, family = binomial)

# summary(sub.glm)

train_sub$predict <- predict(sub.glm, train_sub, type = "response") > 0.2
