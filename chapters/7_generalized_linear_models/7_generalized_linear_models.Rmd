...
par(mfrow = c(1, 3))
    plot(meuse$dist.m, lm_fit$fitted.values, type = "l", ylim = c(0, 1), ylab = "linear fit", xlab = "predictor")
    with(meuse, points(dist.m, lime))
    plot(meuse$dist.m, glm_fit$fitted.values, type = "l", ylim = c(0, 1), ylab = "logistic fit", xlab = "predictor")
    with(meuse, points(dist.m, lime))
    boxplot(dist.m ~ lime, data = meuse, ylab = "predictor", xlab = "response", col = "grey")
```
...
table(s$argillic.horizon) # tabulate the number of argillic horizons observed
...
ssa <- readOGR(dsn = "L:/geodata/soils/soilsa_a_nrcs.shp", layer = "soilsa_a_nrcs") # read in soil survey area boundaries
...
# writeOGR(pedons_sp, dsn = "L:/geodata/project_data/8VIC", "pedon_locations", driver = "ESRI Shapefile") # write shapefile of pedons
```
...
```{r extract}
folder <- "L:/geodata/project_data/8VIC/ca794/"
files <- c(
...
```{r spatial}
data$argillic <- data$argillic == TRUE & data$mrvbf > 0.15 # Include only argillic horizons that only occur on fans. Argillic horizons that occur on hills and mountains more than likely form by different process, and therefore would require a different model.

data_m <- subset(data, select = - c(pedon_id, taxonname, x_std, y_std, describer2))
data_m <- melt(data_m, id = "argillic")
...
aggregate(twi ~ argillic, data = data, median, na.rm = TRUE)
...
test <- glm(argillic ~ ., data = data2, family = binomial(link = "cloglog"))
summary(test)
confusionMatrix(test$fitted.values > 0.5, as.logical(test$y), positive = "TRUE")

test2 <- glm(argillic ~ twi_sc + slope + z2str + tc_2, data = data2, family = binomial(link = "cloglog"))
summary(test2)
```
...
- Discuss the variability of the predictions across the clusters, perhaps different models need to be constructed in each cluster, some cluster appear to be dominanted by specific soil series, this data isn't clean enough (nor are concepts usually) to model series separately however we could use the cluster as an additional model to separate the series

```{r evaluation}
comp <- cbind(data3[c("argillic", "cluster")], pred = predict(test_glm, data3, type = "response") > 0.5)
comp <- na.exclude(comp)
confusionMatrix(comp$pred, comp$argillic, positive = "TRUE")

comp2 <- comp[comp$argillic == TRUE, ]
group_by(comp2, cluster) %>% summarize(sum(argillic), sum(pred), recall = round(sum(pred == argillic) / length(argillic), 2))
```
...
```{r prediction}
r <- predict(geodata_r, test2, progress="text", type = "response")
writeRaster(r1, "test.tif", overwrite = T, progress = "text")
```
