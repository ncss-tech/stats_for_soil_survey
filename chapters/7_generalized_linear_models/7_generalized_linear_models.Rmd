...
```{r}
s$taxonname <- ifelse(grepl("Jumbo", s$taxonname), "Jumborox", s$taxonname) # search for the word "Jumbo" and if found replace with "Jumborox", otherwise keep the original taxonname
s$taxonname <- ifelse(grepl("Helen", s$taxonname), "Helendale", s$taxonname)
s$taxonname <- ifelse(s$taxonname == "Jumborox" | s$taxonname == "Helendale", s$taxonname, "other")
...
s_sub <- subset(s, select = c(taxonname, slope_field, elev_field, surface_total)) # subset quantitative columns and taxonname
s_m <- melt(s_sub, id = "taxonname") # convert s_sub to wide data format
head(s_m)
bwplot(taxonname ~ value | variable, data = s_m, scales = list(x = "free"))

table(s$landform.string, s$taxonname) # cross tabulate landform vs taxonname

round(prop.table(table(s$hillslope_pos, s$taxonname), 2) * 100) # cross tabulate and calculate proportions, the "2" calculates the proportions relative to the column totals
round(prop.table(table(paste(s$shapedown, s$shapeacross), s$taxonname), 2) * 100)
...
table(s$describer, s$taxonname)
round(prop.table(table(s$describer, s$taxonname), 2) * 100)
...
geodata_df <- data.frame(
  as.data.frame(pedons_sp)[c("taxonname", "argillic.horizon", "x_std", "y_std", "describer")],
...
data_m <- subset(data, select = - c(describer, argillic.horizon))
data_m <- melt(data_m, id = "taxonname")
bwplot(taxonname ~ value | variable, data = data_m, scales = list(x = "free"))
...
data$jumborox <- data$taxonname == "Jumborox"
data$helendale <- data$taxonname == "Helendale"

data2 <- subset(data, select = - c(describer, argillic.horizon, test, helendale, taxonname))

test <- glm(jumborox ~ ., data = data2, family = binomial())
summary(test)
confusionMatrix(test$fitted.values > 0.5, as.logical(test$y), positive = "TRUE")

test2 <- glm(jumborox ~ twi_sc + z2str + precip, data = data, family = binomial())
summary(test2)
