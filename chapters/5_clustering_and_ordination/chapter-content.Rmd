...
opts_chunk$set(message=FALSE, warning=FALSE, background='#F7F7F7', fig.align='center', fig.retina=2, dev='png', tidy=FALSE, verbose=FALSE, antialias='cleartype')
...
ex <- sp4[1:4, c('name', 'clay', 'sand', 'Mg', 'Ca', 'CEC_7')]
d <- dist(ex[, c('clay', 'sand')])
m <- round(as.matrix(d))
knitr::kable(ex, caption='A matrix of data: soil horizons (individuals) and associated characteristics.', align = 'c')
```

Similarity among individuals is more conveniently expressed (why?) as distance, or dissimilarity. In the simplest case, dissimilarity can be computed as the Euclidean distance between individuals in property-space. Using two characteristics (dimensions) from the data above, distance is represented as the line connecting any two individuals in property space:

```{r, echo=FALSE, fig.width=5, fig.height=5}
par(mar=c(5,5,1,1))
plot(clay ~ sand, data=ex, las=1, type='n', xlim=c(10, 60), ylim=c(10, 60), xlab='% Sand', ylab='% Clay')
grid()

segments(ex$sand[4], ex$clay[4], ex$sand[4], ex$clay[2], lwd=1, lty=2, col='Orange')
segments(ex$sand[2], ex$clay[2], ex$sand[4], ex$clay[2], lwd=1, lty=2, col='Orange')
segments(ex$sand[1], ex$clay[1], ex$sand[2], ex$clay[1], lwd=1, lty=2, col='RoyalBlue')
segments(ex$sand[2], ex$clay[2], ex$sand[2], ex$clay[1], lwd=1, lty=2, col='RoyalBlue')
arrows(ex$sand[4], ex$clay[4], ex$sand[2], ex$clay[2], lwd=2, col='Orange', length = 0.1, code = 3)
arrows(ex$sand[1], ex$clay[1], ex$sand[2], ex$clay[2], lwd=2, col='RoyalBlue', length = 0.1, code = 3)

text(ex$sand, ex$clay, ex$name, col='black', cex=1.5, font=1, pos = 3)
points(clay ~ sand, data=ex, pch=16, col='black', cex=0.75)

legend('topright', legend=c(m[1,2], m[4,2]), col=c('RoyalBlue', 'Orange'), lty=1, lwd=2, bty='n', title = 'Distance', cex=1.5)
```

A matrix of all pair-wise distances (the distance matrix) looks something like this:
```{r, echo=FALSE, fig.width=5, fig.height=5}
dimnames(m) <- list(ex$name, ex$name)
m
```


This simplistic notion of distance doesn't make much sense when characteristics do not share a common unit of measure, range of values, or when some characteristics are categorical vs. continuous.
...
```{r, fig.width=8, fig.height=4, htmlcap='xxx'}
...
```{r fig.width=5, fig.height=5, htmlcap='xxx'}
...
example reference [@OSD_reference]
