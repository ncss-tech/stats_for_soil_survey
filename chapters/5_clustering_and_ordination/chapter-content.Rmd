...
The agglomerative methods also depend on the choice of [linkage criterion](https://en.wikipedia.org/wiki/Hierarchical_clustering#Linkage_criteria). Some of these criteria include:
  ...
  7. Flexible linkage: adjustable linkage based on parameter `alpha`.  

See [@Kaufman2005], [@Arkley1976], and [@Legendre1998] for a detailed description of these linkage criteria. Selection of linkage criteria can be quantitatively evaluated using the [cophenetic correlation](https://en.wikipedia.org/wiki/Cophenetic_correlation); and index of how well a dendrogram preserves the original pair-wise distances. More on this later.
...
##### Review and Discuss
  ...
  * *Personal opinion:* I usually select divisive hierarchical clustering over the other methods: structures more interpretable.
...
h.avg <- cor(d, cophenetic(agnes(d, method='average')))
h.single <- cor(d, cophenetic(agnes(d, method='single')))
h.complete <- cor(d, cophenetic(agnes(d, method='complete')))
h.ward <- cor(d, cophenetic(agnes(d, method='ward')))

# divisive hierarchical clustering
h.div <- cor(d, cophenetic(diana(d)))

# combine results into a table
res <- data.frame(method=c('average', 'single', 'complete', 'ward', 'divisive'), cophenetic.correlation=c(h.avg, h.single, h.complete, h.ward, h.div))

print(res)
```

Test the "flexible" linkage criteria by iterating over values of `alpha`. Looks like a value near `0.5` would be ideal (e.g. highest cophenetic correlation). Interestingly, this is very close to the cophenetic correlation associated with the "average" linkage criteria.
```{r fig.width=6, fig.height=4}
# init a sequence spanning 0.01 to 1
alpha <- seq(from=0.01, to=1, length.out = 100)
# init an empty vector to store results
r <- vector(mode='numeric', length = length(alpha))

# iterate over values of alpha and compute cophenetic correlation
for(i in 1:length(r)) {
  r[i] <- cor(d, cophenetic(agnes(d, method='flexible', par.meth = alpha[i])))
}

# plot
par(mar=c(5,5,1,1))
plot(alpha, r, type='l', xlab='alpha parameter', ylab='cophenetic correlation', pch=16)
# locate the max coph. corr.
idx <- which.max(r)
# mark this point and label
points(alpha[idx], r[idx], col='red', cex=2, lwd=2)
text(alpha[idx], r[idx], labels = round(alpha[idx], 3), pos=4)
```
...
**TODO: integrate the [clustree package](https://lazappi.github.io/clustree/articles/clustree.html)**
...
Integrate [related tutorial](http://ncss-tech.github.io/AQP/aqp/soil-color-signatures.html)
Explain.
```{r fig.width=12, fig.height=5}
 ...
 * The relative position of points and contours are meaningful; absolute position will vary each time the figure is generated.
