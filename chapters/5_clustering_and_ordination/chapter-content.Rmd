...
**TODO: document missing link between distances / dendrogram: clustering**
...
[**Cluster analysis**](https://en.wikipedia.org/wiki/Cluster_analysis) is a massive topic that deals with the seemingly simple task of finding useful groups within a dataset. This topic and the methods used are also referred to as "unsupervised classification" in the fields of remote sensing and GIS. All of the available algorithms will find groups in a give dataset; however, it is up to the subject expert to determine the following:
  ...
  4. Interpretation of the final grouping based on subject knowledge, and
  5. The possiblity of needing to start over at step 1.
...
The agglomerative methods also depends on the choice of [linkage criterion](https://en.wikipedia.org/wiki/Hierarchical_clustering#Linkage_criteria). Some of these criteria include:

  * Single linkage,
  * Complete linkage,
  * Ward's method,
  * Weighted average linkage, and
  * Flexible linkage.  
...
##### Review and Discuss
  ...
  * When selecting a method, think about it; don't just go "fishing."
...
[Principal component analysis](https://en.wikipedia.org/wiki/Principal_component_analysis) is one of the simplest and most widely used ordination methods. The reduced space ("principal components") are defined by linear combinations of characteristics. The rest of this chapter is focused on [multidimensional scaling](https://en.wikipedia.org/wiki/Multidimensional_scaling) (MDS). Methods of ordination that use ["Non-metric" multidimensional scaling](https://en.wikipedia.org/wiki/Multidimensional_scaling#Non-metric_multidimensional_scaling) (nMDS) attempt to generate a reduced space that minimizes distortion in proportional similarity; i.e., similar individuals are near each other in the reduced space, dissimilar individuals are farther apart.
...
Because this is a complex topic, it is described in a [supplemental set of slides](http://ncss-tech.github.io/AQP/presentations/AQP-num_soil_classification.pdf).
...
The following examples run the functions on the `gopheridge` sample dataset.
...
The go-to functions for hierachrical clustering are as follows:
 
  * `hclust()`: This function is agglomerative, is in base R, requires a distance matrix, and implements most of the commonly used linkage critera.
  * `agnes()`: This function is agglomerative, is in "cluster" package, can perform standardization and distance calculations, and implements more linkage critera.
...
The following example is simple. Many more possibilities are available in the [dendextend package](https://cran.r-project.org/web/packages/dendextend/index.html).
```{r}
...
The following creates simulated data for demonstration puposes, representing two populations: 1) mean = 0, sd = 0.3, and 2) mean = 1, sd = 0.3.
```{r, }
...
### How Many Clusters?
There is no simple answer to the question "How many clusters are in my data?" Some metrics, however, can be used to help estimate a "resonable" number of clusters. The mean [silhouette width](https://en.wikipedia.org/wiki/Silhouette_(clustering)) is a useful index of "cluster compactness" relative to neighbor clusters [@Rousseeuw1987]. Larger silhouette widths suggest tighter grouping.
...
### MLRA Summaries: PRISM and Elevation

**TODO: describe use of ordination / density contours for sampled PRISM data**
...
The following is an investigation of topographic "signatures" in the initial soil mapping project for Sequoia and Kings Canyon, California.
...
x <- read.csv('C:/home/R_code/Stats4SoilScien(2017)/blue.csv', stringsAsFactors = FALSE)

# read-in environmental data
prd <- read.csv('C:/home/R_code/Stats4SoilScien(2017)/blue_env.csv', stringsAsFactors = FALSE)
