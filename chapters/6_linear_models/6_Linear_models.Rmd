...
summary(s$surface_total)

densityplot(~ surface_total + surface_gravel, data = s, auto.key = TRUE)
```
...
```{r landform}
# Landform vs surface_total

test <- aggregate(surface_total ~ landform.string, data = s, function(x) round(quantile(x, probs = c(0, 0.5, 1))))

test[order(test$surface_total[, 2]), ]

s_sub <- subset(s, grepl("fan", landform.string)) # subset fan landforms

densityplot(~ surface_total + surface_gravel, data = s_sub, auto.key = TRUE)


# Hillslope position

test <- aggregate(surface_total ~ hillslope_pos, data = s_sub, function(x) round(quantile(x, probs = c(0, 0.5, 1))))
test[order(test$surface_total[, 2]), ]

# Slope shape

test <- aggregate(surface_total ~ paste(shapedown, shapeacross), data = s_sub, function(x) round(quantile(x, probs = c(0, 0.5, 1))))
test[order(test$surface_total[, 2]), ]


# Surface morphometry, depth and surface rock fragments

s_sub <- subset(s, grepl("fan", landform.string), select = c(surface_total, surface_gravel, bedrckdepth, slope_field, elev_field)) # subset slopes > 15 to just look and fans, and select numeric columns

round(cor(s_sub, use = "pairwise.complete.obs"), 2)

scatterplotMatrix(s_sub, use = "pairwise.complete.obs")
...
test <- aggregate(surface_total ~ describer2, data = s_sub, function(x) round(quantile(x)))
test[order(test$surface_total[, 2]), ]
```
...
folder <- "F:/geodata/project_data/8VIC/"
files <- c(
...
data <- data.frame(
   as.data.frame(pedons_sp)[c("pedon_id", "taxonname", "surface_total", "x_std", "y_std", "describer2", "landform.string")],
...
load(file = "C:/workspace/ch7_data.Rdata")
test <- data
test$frags <- test$surface_total
test$cluster <- as.factor(test$cluster)
test$landform <- ifelse(grepl("fan", test$landform.string), "fan", "hill"); test$landform <- as.factor(test$landform)

test <- subset(test, mrvbf > 0.5 & landform != "hill" & frags < 100, select = - c(pedon_id, taxonname, surface_total, landform.string, x_std, y_std, landform)) # Include only argillic horizons that only occur on fans. Argillic horizons that occur on hills and mountains more than likely form by different process, and therefore would require a different model.
...
round(cor(test[c("frags", terrain1)]), 2)
round(cor(test[c("frags", terrain2)], use = "pairwise.complete.obs"), 2)
round(cor(test[c("frags", climate)]), 2)
round(cor(test[c("frags", landsat)]), 2)
round(cor(test[c("frags", tcaps)], use = "pairwise.complete.obs"), 2)
round(cor(test[c("frags", grad)]), 2)

scatterplotMatrix(test[c("frags", terrain1)])
scatterplotMatrix(test[c("frags", terrain2)])
scatterplotMatrix(test[c("frags", climate)])
scatterplotMatrix(test[c("frags", tcaps)])
scatterplotMatrix(test[c("frags", grad)])

bwplot(frags ~ as.factor(cluster), data = test)

bwplot(twi ~ landform, data = test)
aggregate(twi ~ landform, data = test, median)
test$twi_sc <- abs(test$twi - 13.8)
test$gsi <- with(test, (ls_3 - ls_1) / (ls_3 + ls_2 + ls_1))
test$sw <- cos(test$aspect - 255)
...
```{r}
full <- lm(frags ~ ., data = test) # "~ ." includes all columns in the data set
null <- lm(frags ~ 1, data = test) # "~ 1" just includes an intercept

add1(null, full, test = "F") # using the AIC test the effect of '

null <- update(null, . ~ . + elev) # add twi_sc to the model, 

test2 <- lm(frags ~ elev + gsi, data = test)

test_sub <- subset(test, !(cluster %in% c(2, 6, 7, 8, 13, 15)))

temp <- by(test, list(test$cluster), FUN = function(x) with(x, data.frame(cluster = unique(cluster), r2 = round(cor(frags, predict(test2, x))^2, 2), n = length(frags))))
do.call(rbind, temp)
