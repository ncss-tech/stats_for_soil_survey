...
pedons$surface_gravel <- with(s, surface_gravel - surface_fgravel) # recalculate gravel
...
data <- data.frame(
   as.data.frame(pedons_sp)[c("pedon_id", "taxonname", "argillic", "surface_total", "x_std", "y_std", "describer2", "landform.string")],
...
load(file = "C:/workspace/ch7_data.Rdata")
test <- data
test <- subset(test, mrvbf > 0.15) # Include only argillic horizons that only occur on fans. Argillic horizons that occur on hills and mountains more than likely form by different process, and therefore would require a different model.
test$frags <- test$surface_total
test$landform <- ifelse(grepl("fan", test$landform.string), "fan", "hill")

terrain1 <- c("elev", "slope", "solar", "mrrtf", "mrvbf")
terrain2 <- c("twi", "ch", "z2str", "solar")
climate <- c("precip", "precipsum", "temp")
...
test2 <- subset(test, select = - c(pedon_id, describer2, taxonname, x_std, y_std, landform.string, landform, cluster))
