...
sum(s$frags < 1) # number of samples greater than 100
```
...
```{r landform}
# Landform vs frags
quantile2 <- function(x) round(quantile(x, probs = c(0.05, 0.5, 0.95))) # create custom quantile function

test <- aggregate(frags ~ landform.string, data = s, quantile2)
arrange(test, frags[, 2], decreasing = TRUE) # sort data frame by column using plyr package function

# or

# test[order(test$surface_total[, 2], decreasing = TRUE), ] # sort using base package function
...
arrange(test, landform, describer2, frags[, 2], decreasing = TRUE)
```
...
load(file = "C:/workspace/ch7_data.Rdata")
train <- data
train <- subset(train, frags > 0 & frags < 100, select = - c(pedon_id, taxonname, landform.string, x_std, y_std, argillic.horizon)) # Include only argillic horizons that only occur on fans. Argillic horizons that 
train$fragst <- with(train, log10((frags / 100) / (1 - (frags / 100))))

occur on hills and mountains more than likely form by different process, and therefore would require a different model.
...
round(cor(train[c("fragst", landsat)], use = "pairwise"), 2)
...
train$predict <- predict(fragst_lm, train)

with(train, plot(fragst, predict, xlim = c(0, 100), ylim = c(0, 100)))
abline(0, 1)

sqrt(mean((train$fragst - predict(fragst_lm, train))^2, na.rm = T)) # RMSE = root mean square error
mean(abs(train$fragst - predict(fragst_lm, train)), na.rm = T) # MAE = mean absolute error

temp <- by(train, list(train$cluster2), FUN = function(x) with(x, data.frame(cluster = unique(cluster2), rmse = round(sqrt(mean((fragst - predict(fragst_lm, x))^2, na.rm = TRUE))), n = length(fragst))))
...
full <- lm(fragst ~ ., data = train_sub) # "~ ." includes all columns in the data set
null <- lm(fragst ~ 1, data = train_sub) # "~ 1" just includes an intercept

add1(null, full, test = "F") # using the AIC test the effect of '


temp <- by(train, list(train$cluster2), FUN = function(x) with(x, data.frame(cluster = unique(cluster2), r2 = round(cor(fragst, predict(test, x), use = "pairwise.complete.obs")^2, 2), n = length(fragst))))
temp2 <- do.call(rbind, temp)
