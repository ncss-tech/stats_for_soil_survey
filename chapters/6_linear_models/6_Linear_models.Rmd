...
densityplot(~ surface_cobbles + surface_gravel + surface_fgravel, data = s, auto.key = TRUE)
...
```{r landform}
# Landform vs frags
quantile2 <- function(x) round(quantile(x, probs = c(0.05, 0.5, 0.95)))
...
arrange(test, landform, frags[, 2]) # sort data frame by column using plyr 

densityplot(~ frags + surface_cobbles + surface_gravel | landform, data = s, auto.key = TRUE, scale = list(relation = "free"))
...
arrange(test, describer2, landform, frags[, 2])
```
...
ssa <- readOGR(dsn = "F:/geodata/soils/soilsa_a_nrcs.shp", layer = "soilsa_a_nrcs") # read in soil survey area boundaries
...
idx <- aggregate(mast ~ cluster, data = data, function(x) round(mean(x), 2))
...
train <- subset(train, frags < 100, select = - c(pedon_id, taxonname, landform.string, x_std, y_std)) # Include only argillic horizons that only occur on fans. Argillic horizons that occur on hills and mountains more than likely form by different process, and therefore would require a different model.

terrain1 <- c("slope", "solar", "mrrtf", "mrvbf")
terrain2 <- c("twi", "z2str", "ch")
climate <- c("elev", "precip", "precipsum", "temp")
landsat <- paste0("ls_", 1:6)
pcomps <- paste0("pc_", 1:6)
tcaps <- paste0("tc_", 1:3)
grad <- c("k", "th", "u")

round(cor(cbind(train$frags, train[terrain1]), use = "pairwise.complete.obs"), 2)
round(cor(cbind(train$frags, train[terrain2]), use = "pairwise.complete.obs"), 2)
round(cor(cbind(train$frags, train[climate]), use = "pairwise.complete.obs"), 2)
round(cor(cbind(train$frags, train[landsat]), use = "pairwise.complete.obs"), 2)
round(cor(cbind(train$frags, train[pcomps]), use = "pairwise.complete.obs"), 2)
round(cor(cbind(train$frags, train[tcaps]), use = "pairwise.complete.obs"), 2)
round(cor(cbind(train$frags, train[grad]), use = "pairwise.complete.obs"), 2)

spm(train[c("frags", terrain1)])
spm(train[c("frags", terrain2)])
spm(train[c("frags", climate)])
spm(train[c("frags", pcomps)])
spm(train[c("frags", tcaps)])
spm(train[c("frags", grad)])

bwplot(frags ~ cluster, data = train)
bwplot(frags ~ cluster2, data = train)
...
```{r}
full <- lm(frags ~ ., data = train) # "~ ." includes all columns in the data set
null <- lm(frags ~ 1, data = train) # "~ 1" just includes an intercept

add1(null, full, test = "F") # using the AIC test the effect of '

null <- update(null, . ~ . + pc_2) # add twi_sc to the model, 

test <- lm(frags ~ pc_2 + pc_1 + pc_5 + twi + elev, data = train)
test <- lm(frags ~ twi + elev, data = train)
test <- lm(frags ~ elev + gsi, data = train)
test <- lm(frags ~ precipsum + tc_3 + tc_2 + twi + elev + cluster, data = train)

sqrt(mean((train$frags - predict(test, train))^2, na.rm = T))
mean(abs(train$frags - predict(test, train)), na.rm = T)

temp <- by(train, list(train$cluster2), FUN = function(x) with(x, data.frame(cluster = unique(cluster2), rmse = round(sqrt(mean((frags - predict(test, x))^2, na.rm = TRUE))), n = length(frags))))
...
train_sub <- subset(train, mast.1 < 23)

full <- lm(frags ~ ., data = train_sub) # "~ ." includes all columns in the data set
null <- lm(frags ~ 1, data = train_sub) # "~ 1" just includes an intercept

add1(null, full, test = "F") # using the AIC test the effect of '


temp <- by(train, list(train$cluster2), FUN = function(x) with(x, data.frame(cluster = unique(cluster2), r2 = round(cor(frags, predict(test, x), use = "pairwise.complete.obs")^2, 2), n = length(frags))))
temp2 <- do.call(rbind, temp)
plot(1:nrow(temp2), temp2$r2, type = "l")

r <- predict(geodata_r, test, progress="text", type = "response")
writeRaster(r, "test_frags4.tif", overwrite = T, progress = "text")

plot(raster("C:/workspace/test_frags.tif"))
plot(ca794, add = TRUE)
```
