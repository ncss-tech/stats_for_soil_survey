...
test <- lm(frags ~ precipsum + tc_3 + tc_2 + twi + elev, data = train)

sqrt(mean((train$frags - predict(test, train))^2, na.rm = T))

temp <- by(train, list(train$cluster2), FUN = function(x) with(x, data.frame(cluster = unique(cluster), r2 = round(cor(frags, predict(test, x))^2, 2), n = length(frags))))
do.call(rbind, temp)
