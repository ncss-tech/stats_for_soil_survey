...
View(pedons) # Examine the makeup of the data we imported from NASIS.
...
sapply(s[grepl("surface|frags", names(s))], function(x) round(summary(x))) 

# number of samples greater than 100
sum(s$surface_total_frags_pct > 100) 
...
s_fan <- subset(s, landform_generic == "fan", select = c(frags, surface_gravel, bedrckdepth, slope, elev))
s_hill <- subset(s, landform_generic == "hill", select = c(frags, surface_gravel, bedrckdepth, slope, elev))
...
train <- subset(train, frags > 0 & frags < 100, select = - c(peiid, taxonname, landform, x, y, argillic.horizon)) # exclude frags greater than 100 and less than 1, and exclude some of the extra columns
...
terrain1 <- c("slp", "solar", "mrrtf", "mrvbf")
...
train2 <- train[c("fragst", terrain1, terrain2, climate, pc, tc, rad)]
...
# Fit null model
null <- lm(fragst ~ 1, data = train2) # "~ 1" just includes an intercept
...
test <- regsubsets(fragst ~., data = train2, nvmax = 10, method = "forward")
adjR2 <- data.frame(
