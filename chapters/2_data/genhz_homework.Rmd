...
For your homework, we ask you to assign _generalized horizon labels_ to a set of pedons from _your_ area of responsibility. You will assign these labels to determine a Range in Characteristics (RIC) for one (or more) properties of your choice. You will generate a RIC for each _soil-property X generalized-horizon-label_ combination.

This document takes you through a demo of the homework assignment using a subset of the `loafercreek` dataset from the _soilDB_ package. You are encouraged to run through the code with `loafercreek` before attempting it on your own data. There are some __Homework Tips__ at the end of the document to help you get going on your own analyses.

_Do your best_ and _have fun_ with this. Try to customize the analysis for _your_ soils.

This assignment integrates several R/data analysis skills as well as brings on the "Great Unknown" of NASIS data inputs from across the country. So there is much room for learning _new_ things ... and dealing with _new problems_. If it does not work right at first, _do not be discouraged_. 
...
1. Query your Local NASIS database to __fill your selected__ set with some pedons.
...
5. Attempt to __write a set of regular expressions__ (you need one per prototype horizon) to do the mapping you thought about in *#4*. Use `generalize.hz()` to apply the patterns you devise to your data.

6. __Cross-tabulate__ your generalized horizon labels against the field horizon designations.

7. __Check if any horizon designations were _not_ assigned a prototype horizon__ ('not-used'). At a minimum answer the question: "_Which ones [were not assigned]?_" Bonus points if you can answer "_Why?_"
...
9. __Do a statistical summary on your horizon groups.__ You could use the example given for `loafercreek` (for summarizing mean/sd/min/max clay content) or do something else. At a minimum for a proper "summary" you should return _at least one value_ for _each prototype horizon_. That value can, _for some groups_, be `NA`, depending on your data and summary statistic of choice.

_Be prepared to discuss issues you may have had while implementing generalized horizon labels for horizon RICs in your data. In particular, what "decisions" did you have to make that might have influenced your final "correlations"?_
...
Then you will apply the same strategy to NASIS pedons from _your_ area of repsonsibility.
...
# Why _Generalized Horizon Labels_?
...
When creating summaries of data, we need a way to relate observations of _particular_ horizons from _particular_ pedons back to the _typical_ horizons found in the "group" (e.g. a series, or a component) the data belong to. 

Maybe you could look at all the _unique_ horizon designations in your data?
...
Even with our 30 pedon subset we have a lot. Maybe the approach would work if the horizon designation scheme was _very consistent_ across all observations (say, same describer or very similar soils).

But here we have `r length(unique(pedons$hzname))` groups... I doubt you've ever seen a OSD/series/component with that many RICs/layers... :)

We need a way to "simplify" this set of unique groups in order to support correlation of pedon data to the series horizon RICs we are trying to derive. 

Depending on things like depth class and the nature of the parent material, the number of horizon RICs provided in a series is variable. _However, I can't think of any legitimate cases where there are more than 10, and more than 5 is probably pushing it for most soil concepts._ Many series concepts, especially older ones, provide RIC for only a couple _very_ generalized horizons (say, just an A and a Bt in a very deep soil) and avoid providing ranges for transitional horizons/vertical subdivisions. 

More "modern" descriptions might have more layers broken out, but there are diminshing returns to this. More subdivision does not necessarily mean better/more accurate representation of the aggregate entity.

The great thing about the _generalized horizon label_ approach is that you can "test" the effect of adding additional groups on the RIC for all of your groups together. Then you can decide if it is beneficial to have more or less groups based on the data you have.

One way of doing this is by matching patterns in the field horizon designations to correlate them to simpler labels. 

We call assignment of generalized labels to horizons: _micro-correlation_. 

_Correlation_ of observations into more generalized groups typically _increases_ the number of observations in each horizon label/group, while _simplifying_ the grouping scheme. With generalized horizon labels, correlation decisions are being made on a horizon basis (not pedon), so we call it "micro."

### Correlation v.s. Micro-correlation

_Correlations_ are made at the pedon level, and essentially determine whether a particular profile is a good representative of a series (for instance). That is: is "Pedon X" the _series_? the _series family_? a _taxadjunct_? A _similar soil_? Something else?

_Micro-correlations_ are made at the horizon level and determine what data from each pedons contributes to which range in characteristics. A simple micro-correlation would be: "this AB horizon is more like an A than a Bt, so [given the two options] it goes in the A group rather than Bt". 

You can (and should) in practice look at more than just horizon designation. For instance: Horizon Y should be included in the RIC for Bt horizons of Series Z because it has clay films, a 5YR hue and 32% clay.

However, programatically grouping by horizon designation is an excellent way to begin to explore the properties of a set of soils. It is also appealing because we often give ranges in terms of (groups of) horizon designations. As soil scientists we put a lot of effort into our descriptions/describing changes in profiles with a change in horizons... so it is a logical way to attempt to stratify our data.

#### Micro-correlation

To "micro-correlate", you first need to come up with a "prototype" scheme for horizonation for the soil you are studying. 

This could be (a generalization of) the horizons that occur in the OSD/Type location/TUD pedon, for instance. Then you need to produce a set of patterns that map the field-observed horizon designations onto your prototype. 
...
We created 4 generalized horizon labels ('A','BA','Bt','Cr'), and 4 regular expression patterns to assign data to those labels ('^A', '^B.*[^t]$', '.*B.*t.*', 'Cr|R'). 
...
#### Micro-correlation Theory - applied to Loafercreek

Since there are only a handful of observations for the C's and O's (4 and 2 of each, respectively) they may not be particularly representative for the "Loafercreek series." If that is the case, it is probably fine that they are _not_ included in a group label (they are 'not-used'). The lack of clay films is a commonality between "BC" and "C" could be combined?

A note like "some pedons have a thin, discontinuous layer of slightly decomposed grass, forb or oak litter" would be sufficient to "handle" the occurence of Oi in an OSD or TUD description. Loafercreek is a thermic blue oak grassland soil and it doesn't _usually_ have much of an O horizon. Therefore, we aren't terribly concerned about leaving O horizons out of the series RIC. They often are more like _undecomposed_ litter/duff layers anyway (rather than a true "Oi"). A note might not even be needed.
...
We will update our regular expression patterns to _include_ a mapping for the BC and C horizons in the input data. This is to illustrate that the development of _generalized horizon label_ patterns is an _iterative process_. We will look at the data for that group to see if it looks different from our other groups.

For the new "BC prototype horizon" pattern we match all horizons that contain "C" and have zero or more characters that are NOT "r" or "t" and put them in the "C" group. 

We apply the patterns as before, but create another generalized horizon label variable `pedons$newgenhz2` to hold the new result. 

```{r}
# create 5 generalized horizons: A, upper transitional, argillic, lower-transitional and bedrock
new.genhz.labels.v2 <- c('A','BA','Bt','BC','Cr')
...
As you can see, the 5 "BC" and 4 "C" horizons that were 'not-used' before are now correlated to the "BC" group.
...
# plot profile sketches
par(mar=c(0,0,0,0))
plotSPC(pedons, name='hzname', color='newgenhz2', print.id=FALSE, cex.names=0.8, axis.line.offset=-4)
```

You can see some trends that shake out when the data are simplified this way. 

Here are a few thinks that are evident for the loafercreek example:
...
res <- lapply(genhz.list, FUN = function(d) {
  ...
  # CHECK FOR NaN mean result; if NaN, na.rm removed all records. Return NA
  if(!is.nan(clay.mean)) {
...
*Question:* _What values might be better for a RIC (LOW-RV-HIGH) than minimum-mean-maximum? Why_

*Bonus question:* _What do you notice about the clay content mean, standard deviation, minimum and maximum for each of our generalized horizon labels? Is that what you expected? Why?_
...
For proof of concept in this exercise there is no need for a _MANY_ of observations. _Roughly n=30_ should be sufficient for a "realistic" demonstration.
