...
Then you will apply the same strategy to NASIS pedons from _your_ area of responsibility.
...
More "modern" descriptions might have more layers broken out, but there are diminishing returns to this. More subdivision does not necessarily mean better/more accurate representation of the aggregate entity.
...
For the new "BC prototype horizon" pattern we match all horizons that contain "C" and have zero or more characters that are NOT "r" or "t" and put them in the "BC" group. 
...
par(mar=c(0,0,0,0))
plotSPC(pedons[1:20, ], name='hzname', color='newgenhz2', print.id=FALSE, cex.names=0.5, axis.line.offset=-2)
```
