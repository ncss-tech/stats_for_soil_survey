 ...
 * [2.7 References](#ref)
 * [2.8 Additional resources](#resources)
 ...
 **Query and load some pedon data into your NASIS selected set**
 TODO: show screen shots of the results returned - necessary?
...
The following is from another tutorial.....in the aqp package....rework and include portions of this?
...
####TODO: add section on options that can be set within fetchNASIS()
...
# remove any sites lacking standard coordinates
...
# plot again this time with county boundaries for your state - enter your state!!!
...
```
Question:  What is a potential problem with this operation?  What has not been accounted for?

###Extended data functions in soilDB

Additional data related to both site and horizon information can be fetched using the `get_extended_data_from_NASIS()` function.  The reason that this data is not automatically brought into R is that in most cases these data elements are related to the site or horizon data as one to many relationships.  Multiple diagnostic features could exist within one pedon for example.

TODO: show summary of all the extended date elements that are currently pulled into R.  These function examples will only work if your pedon data has carbonates or argillic horizons.....work up a parallel example going for depth to argillic using gopheridge or loafercreek datasets (neither of these sets have carbonates).

###Thicknesses of diagnostic features

####TODO: explain the treatment of diagnostic features as boolean fields in the site data

We could have used the following code to pull the upper depth to calcium carbonates using the calcic diagnostic feature or the secondary carbonates diagnostic feature.  However, this is where data consistency hits home.  We could use those fields if they have been consistently populated for all pedons that we are working with in our selected set.  As you start working with larger pedon data sets you will quickly see there can be great inconsistencies in the way that the data was populated by different people in different offices on different surveys over different timeframes.

The following is an example of how you could use the diagnostic features (if fully populated) to determine feature thicknesses:
...
TODO: points to touch on further......
...
###<a id="refs")></a>2.7  References and useful links




###<a id="resources")></a>2.8  Additional resources

[R tutorials](http://cran.r-project.org/doc/contrib/Verzani-SimpleR.pdf)
