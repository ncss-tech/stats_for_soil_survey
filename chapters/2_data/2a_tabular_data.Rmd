...
# 2.0 Getting Acquainted With the SoilDB Package
...
### 2.021 Some Issues With Pedon Data

- Making and documenting observations of soil require hard work. Digging is difficult, and writing soil descriptions is time consuming!
    - Our confidence in observations commonly weakens with the with depth of the material described
...
The following examples are meant to be copied from this document and pasted into **R,** where they can run interactively. Comments (green text with "#" sign at left) briefly describe the function of the code in each line. Further documentation on objects and functions from the `aqp` package can be accessed by typing `help(soilDB)` or `help(aqp)` at the R console. The more general form for a help requests is `?function_name`. 
  
## 2.11 Classes of Objects Used in R
 
One of the most versatile things about R is that it can manipulate and work with data in many ways.  Below are examples of how to create and reference information in several data types that are commonly used in working with soil data. Within the R session, *objects* contain information that is loaded from files, extracted from NASIS, created on the fly, or calculated by some function. If none of the base classes are sufficient for a task, it is possible to define custom classes. The `SoilProfileCollection` is one such example. More on this later.
...
#### 2.1111 Referencing elements of a vector
...
#### 2.1112 Vectorized evaluation: Implicit looping
...
Dataframes are central to most work in R. They describe rectangular data, which can be thought of as a spreadsheet format with rows and columns. Each column in the dataframe is constrained to a single data type: numeric, datetime, character, Boolean, and so on. Note that each column of a dataframe is a vector, and all column vectors must be the same length (hence the adjective "rectangular").
...
#### 2.1121 Referencing within dataframes
...
How would I remove a vector or column from a dataframe?
...
# 2.2 Using the SoilDB Package to Load NASIS Pedon Data
 
## 2.21 Set Up an Open Database Connectivity (ODBC) Connection to NASIS  

After seeting up an ODBC connection, you can use R to access data from a selected set defined in your local NASIS database. See this job aid: [**How to Create an ODBC Connection and Setup SoilDB for Use with R**](https://r-forge.r-project.org/scm/viewvc.php/*checkout*/docs/soilDB/setup_local_nasis.html?root=aqp).
...
### 2.221 Additional Convenience Functions in the SoilDB Package
...
- **fetchKSSL()**
    - Gets KSSL data via BBOX, MLRA, or series name query, from the SoilWeb system.
        - For more information, see the following tutorial:
...
### 2.222 Data Checks Run by the `fetchNASIS()` Function
...
d
```
Note the issue above. The bottom depth of the A horizon and the upper depth of the Bt1 horizons should be the same: either 30 or 38 cm. The correct depth needs to be determined.

- **Missing lower horizon depths.**.—Offending horizons are fixed by replacing the missing bottom depth with the top depth + 2 cm. In the case of the profile shown above, a bottom depth of 137 cm would be inserted where the depth is missing.

- **Sites missing pedon records**.—Data without corresponding horizons are not loaded.


#### 2.2221 How can I find the site ID's where these errors occur and fix them in NASIS?
...
For more information on the design of soilDB functions, go to the following documentation: [**Introduction to soilDB**](https://r-forge.r-project.org/scm/viewvc.php/%2acheckout%2a/docs/soilDB/soilDB-Intro.html?root=aqp).


### 2.223 Options That Can Be Set Within fetchNASIS()
...
## 2.23 Structure of Pedon Data in a Soil Profile Collection (SPC)

### 2.231 The `Gopheridge` Sample Dataset
...
Before proceeding, you may be it helpful to review [**SoilProfileCollection object introduction**](https://r-forge.r-project.org/scm/viewvc.php/*checkout*/docs/aqp/aqp-intro.html?root=aqp). This tutorial provides an excellent overview of how the SPC object is constructed. Also, the manual pages for `soilDB` and `aqp` are accessible (click **index** at the bottom of the **Help** tab in RStudio) by entering the following into the R console: 
...
#### 2.2311 Quickly generate sketches from a `SoilProfileCollection` object
...
#### 2.2312 Subset example of the data in the site-level portion of the SPC:
...
#### 2.2313 Subset example of the data in the horizon-level portion of the SPC:
...
#### 2.2314 Follow along with your own data
...
How can I find out how many site and horizon records are in the data you just loaded?

## 2.24 Viewing Pedon Locations
### 2.241 Plotting Geographic Data Directly in R

**Quick check:** Does the data plot roughly where you would expect it?
 
Plotting the data directly as an R graphic can give you some idea of how the data look spatially and whether their distribution approximates what you would expect.  Typos are relatively common when coordinates are manually entered. Viewing the data spatially is a quick way to see if any points plot far outside of the geographic area of interest and therefore clearly have an error.
...
### 2.242 Displaying Pedon Data in Google Earth
Google Earth is a powerful data viewer for point data.  Geographic data is displaye in Google Earth using the Keyhole Markup Language (KML) format.  Using the [plotKML](https://cran.r-project.org/web/packages/plotKML/plotKML.pdf) package, you can easily create a KML file to inspect and view in Google Earth.  See the related material in this tutorial: [**Export Pedons to Google Earth**](https://r-forge.r-project.org/scm/viewvc.php/*checkout*/docs/soilDB/export-points-from-NASIS-to-Google-Earth.html?root=aqp). 

### 2.243 Exporting Pedon Data to an ESRI Shapefile
Another way you can view the data is to export a shapefile from R.  For further information, see this tutorial: [**Export Pedons to Shapefile**](https://r-forge.r-project.org/scm/viewvc.php/*checkout*/docs/soilDB/export-points-from-NASIS.html?root=aqp).

#### 2.2431 Follow along with your own data
...
Use the following script with the data loaded from your local NASIS selected set:
...
# 2.3 Working with SPC Data in R

## 2.31 Summarizing Data
...
### 2.311 Filtering Data Using Pattern Matching
...
Notice below that the `grep()` also has an ***invert option***, which is specified as either true or false (the default is false).  When set to true, this option is very useful for excluding the results of the pattern matching process by inverting the selection.
...
Do a quick graphical check that the "lithic" profiles are selected. Plot them in R using the `SoilProfileCollection` "plot" method (e.g., specialized version of the generic `plot()` function).
...
#### 2.3111 Additional syntax options for REGEX pattern matching
...
### 2.312 Filtering Data by Specifying a Criteria Using the `which()` Function
...
#### 2.3121 Additional syntax options for use in `which()` criteria
...
### 2.313 Extracting Site and Horizon Data
...
### 2.314 Review of Data Checks Run by `fetchNASIS()`
...
### 2.321 Defining a Function
Functions bundle operations and can come in the form of small helper functions or involve larger multi-step processes that can be re-used.  The following focuses on functions that work with the horizon data in the SPC.  The general workflow typically goes like this:
...
#### 2.3221 Example using the Gopheridge sample dataset
...
You still need to summarize this to get the upper depth from multiple "Bt" horizons per profile. Also, note that additional variable were returned by the function.
...
You still need to reduce this to one depth value per profile. The `ddply()` function conveniently iterates over groups of rows in a dataframe and computes summaries (similar to `GROUP BY` in SQL). The group-wise summaries are re-combined into a new dataframe along with ids for each group.
...
```
**Questions**                                                                                                                                          
**What is a potential problem with this operation?** 
...
#### 2.3222 Follow along with your own data

In the following example, you will use pedon data to model the depth to calcium carbonate.  
...
Consider examples where you truncate the thickness or are interested in summarizing the data for a depth zone, for example, 25 to 100 cm.  What is the weighted average clay for the 25 to 100 cm thickness?

Simple example: `slab(f, fm= peiid ~ clay, slab.structure=c(25,100), slab.fun=mean, na.rm=TRUE)`.

Excellent examples using the `slice()` and `slab()` functions in the AQP package can be found at:
...
## 2.33 Getting Additional Data: Extended Data Functions
...
### 2.331 Deriving Thicknesses of Diagnostic Features

#### 2.3311 Boolean diagnostic feature columns in the site data
...
#### 2.3312 Follow along with your own data
...
What can I do with the Boolean diagnostic feature data?

### 2.332 Diagnostic Feature Diagrams
...
#### 2.3321 Follow along with your own data
...
Note: If the data includes more than 20–30 pedons, the script might generate figures that are very hard to read. You also need to be certain that pedon diagnostic feature were populated in your data.
...
For more information on generating diagnostic feature diagrams, go to the following tutorial:
...
## 2.34 Common Challenges in Working with Pedon Data
 ...
 - Location confidence
    ...
    - Reasonable accuracy for GPS values at the time of data collection
    
### 2.341 Meeting the Challenges
