...
Most functions in R are *vectorized*. This means that operations such addition (the `+` function) on vectors will automatically iterate through every element. An example is more informative than further description.
...
l <- list('food'=c('apples', 'bread', 'cheese'),
     'drinks'=c('wine', 'water', 'beer', 'milk'),
...
# access the element named 'drinks'
l[['drinks']]
...
# a list of lists
list('pedon_1'=list('top'=c(0,10,25,55), 'bottom'=c(10,25,55,76), 'pH'=c(6.8,6.6,6.5,6.4)))
```
...
# multiply every combination of the sequences 1:10 and 1:10
outer(1:10, 1:10, '*')
```


 DEB: I suggest ommitting this, too much for this class and we don't have any examples.
 - **Arrays**
...
DEB: should we add links to relevant tutorials here?
...
DEB: ack, this doesn't work for me ---
...
Such a string can then be copy/pasted back as a concatenated string or could even be used as string for NASIS list queries. The `dput()` function is also helpful when sending questions or examples to colleagues via email.

### Logical evaluations using `ifelse()` (this may be too much... more suited to later chapters?)

Often there are operations in which we need to evaluate the site or horizon data and assign some variable using a logic statment.  One way to do this is to use the `ifelse()` function.  The following shows some examples of how to do this. Note that this function is vectorized and therefore will implicitly loop over the elements of a vector.
...
# plot again, this time with diagnostic features ordered occording to co-occurence
...
If there are more than 20-30 pedons, the following code may generate figures that are very hard to read.
