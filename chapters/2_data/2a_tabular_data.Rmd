...
### Resources for Learning the R Language
 ...
 * [**Introduction to R, Notes on R: A programming environment for data analysis and graphics, version (3.2.3, 2015-12-10)**](https://cran.r-project.org/doc/manuals/r-release/R-intro.pdf)
...
So how can we visualize soil pedon data from NASIS? 
...
- **from = 'pedons' or 'components'** 
    - This option allows you to select which data you want to load from the local NASIS database. 
...
#### Additional syntax options for use in `which()` criteria
  ...
  - `%in%` Equivalent to `IN ()` in SQL. Can use `c()` to concatenate lists of vectors.
    - Example:   `which(f$taxpartsize %in% c('loamy-skeletal', 'sandy-skeletal'))`
...
plot(soitemp ~ doy, data=d, type='p', xlim=c(1, 366), ylim=c(-5, 30), xlab='Day of Year', ylab='Soil Temperature at 50cm (deg C)', las=1)
```
