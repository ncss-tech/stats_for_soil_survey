...
Summary via all pixels vs. sample.
```{r}
...
The extracted values are now part of the "auburn" `SoilProfileCollection` object. Does there appear to be a relationship between soil morphology and "effective precipitation"? Maybe.
...
Get an approximate extent for the [Amador](http://casoilresource.lawr.ucdavis.edu/sde/?series=amador) soil series from [SoilWeb](https://casoilresource.lawr.ucdavis.edu/see/).
```{r}
amador <- seriesExtent(s = 'amador')
class(amador)
```

Generate 100 sampling points within the extent, using a hexagonal grid. These will be used to extract raster values.
```{r}
...
par(mar=c(1,1,1,1))
plot(maat, ext=extent(s), main='MAAT, Amador Extent, Sampling Points')
...
See the [series extent mapping](http://ncss-tech.github.io/AQP/soilDB/series-extent.html) tutorial for more ideas.
...
http://ncss-tech.github.io/example-reports/mu-comparison/CA630-mu-comparison.html

http://ncss-tech.github.io/example-reports/mu-comparison/MLRA-comparison-report.html
