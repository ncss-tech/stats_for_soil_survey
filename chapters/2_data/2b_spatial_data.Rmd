...
While it is possible to hand-make `sp` class objects, it is usually faster to import existing raster or vector files from disk. More on this soon. For the sake of demonstration we will make a `SpatialPointsDataFrame` object. This is analogous to the point type feature class or shapefile.
...
See the [sp gallery](https://edzer.github.io/sp/) and vignettes [1](https://cran.r-project.org/web/packages/sp/vignettes/intro_sp.pdf) [2](https://cran.r-project.org/web/packages/sp/vignettes/over.pdf) for a much more detailed description of the `sp` classes.
...
This module will not cover importing or exporting raster data via `rgdal` package as the `raster` package provides a much more convenient interface to the GDAL library.
...
Import data from shapefile stored at `E:/gis_data/ca630/pedon_locations.shp`:
```{r eval=FALSE}
x <- readOGR(dsn='E:/gis_data/ca630', layer='pedon_locations')
```

Note that in the second example the trailing "/" is ommitted from the `dsn` (data source name) and the ".shp" suffix is ommitted from the `layer`.
 ...
 * operations on `sp` class objects (e.g. CRS transformations) are relatively slow
    ...
    + the `sp` package is being re-imagined as part of the `sf` library, it is likely much more efficient
 * it is not possible to efficiently "warp" raster data as `sp` class objects
...
```{r raster, eval=FALSE, message=FALSE, warning=FALSE}
...
```{r raster1, eval=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
...
```{r raster2, eval= FALSE, message=FALSE, warning=FALSE}
