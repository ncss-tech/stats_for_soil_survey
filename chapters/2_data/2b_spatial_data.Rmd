...
# convert extent (bounding-box) of MAAT raster into a SpatialPolygons object
poly <- as(extent(maat), 'SpatialPolygons')

# demonstrate some of the `sp` sampling methods
par(mfcol=c(2,3))
for(i in c('random', 'stratified', 'regular', 'hexagonal', 'clustered', 'nonaligned')) {
  plot(poly, main=i)
  points(spsample(poly, n = 50, type=i))
}
```
