...
In soil survey we're typically interested in the values for spatial data at point locations or whole polygons. This gives us information on the geomorphic setting of our soil observations. With this information we would like to predict the spatial distribution of soil properties or classes at unobserved sites. To extract spatial data R has several packages which provide similar functionality to other GIS programs, but also significantly streamline the process for generating raster maps from statistical models.


### Extracting point data from a raster

To extract point data using R, either the sp or raster packages can be used. For large datasets it is best to use the raster package, because it doesn't require reading the raster files into the computer's random access memory (RAM).
...
Zonal statistics summarize the spatial data that overlay a polygon. This can be accomplished via two methods. The most common method (i.e. census survey) provided by most GIS computes a statistical summary of the rasters that overlay a polygon. This approach is generally faster and computes a summary each cell within a raster. An alternative approach is too take a collection of random samples from each polygon. While this approach is generally slower and does not sample ever cell that overlaps a polygon it does offer certain advantages.
...
# test <- raster("F:/geodata/project_data/8VIC/sdat/ned30m_8VIC.sdat", ext = extent(rs), crs = crs(rs), res = res(rs))  # create blank raster
# 
