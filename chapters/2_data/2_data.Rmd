...
Zonal statistics summarize the contents of a polygon. This can be accomplished via two methods. The most traditional method simply counts the number of occurances of each value within a polygon.
...
test <- raster(extent(ca794), ext = extent(rs), crs = crs(rs), res = res(rs))  # create blank raster
writeRaster(test, file = "M:/geodata/project_data/8VIC/sdat/ca794.tif", progress = "text", overwrite = TRUE)
gdal_rasterize("C:/workspace/ca794.shp", "M:/geodata/project_data/8VIC/sdat/ca794.tif", a = "mukey2", verbose = TRUE)
