...
table(d_cat$predicted > 0.5, d_cat$observed)

# or

library(caret)
cm <- confusionMatrix(d_cat$predicted > 0.5, d_cat$observed, positive = "TRUE")
cm$table
cm$overall
round(cm$byClass, 2)

# Trade Precision for Sensitivity by Varying the Threshold 
cm <- confusionMatrix(d_cat$predicted > 0.4, d_cat$observed, positive = "TRUE")
cm$table
cm$overall["Accuracy"]
round(cm$byClass, 2)
```
