...
author: "Skye Wills and Stephen Roecker"
date: "`r Sys.Date()`"
output:
  html_document:
    number_sections: yes
...
- Re-sample 10 times, each time calculate mean and variance.
...
The correctness of the parameter being predicted by the model (soil taxa, property etc.). Ideally this is done with an independent set of data. 

Error Metrics                   | Description              | Formula                             |
--------------------------------|--------------------------|-------------------------------------|
**Numerical**                   |                          |                                     | 
Coefficient of variation ($R^2$)|  % of variance explained | cor(predicted, observed)^2          | 
Mean Error (ME)                 | average error            | mean(predicted - observed)          |
Root Mean Square Error (RMSE)   | average residual         | sqrt((mean(predicted - observed)^2))| 
**Categorical**                 | derivatives of confusion matrix |                                |
Overall Accuracy                | % of observations that were correctly classified, for all classes                                                                     | (TP + TN) / (TP + TN + FP + FN) |
Precision (i.e. user's accuracy, errors of commission (Type II)                                                                               | % of observations that were correctly classified, for an individual class                                                              | TP / (TP + TN)                      |
Sensitivity (i.e. producer's accuracy, errors of omission (Type I))                                                                          | % of predictions that were correctly classified, for an individual class                                                              | TP / (TP + FP)                      |

Confusion Matrix | Observed           |                     |
-----------------|--------------------|---------------------|
**Predicted**    | Yes                | No                  |
Yes              | True Positive (TP) | False Positive (FP) |
No               | False Negative (FP)| True Negative (TN)  |


## Exercise - Numeric Error Metrics
```{r}
...
d_num <- data.frame(
  depth = 21:60,
  slope = 60:21 + rnorm(40, mean = 0, sd = 10)
  )
...
## Exercise - Categorical Error Metrics
```{r}
# Create a Ficticous Data-set
test <- rep(c("alpha", "beta"), 100)

d_cat = data.frame(
  predicted = sample(test, 20), 
  observed  = sample(test, 20)
  )

# Compute Confusion Matrix
table(d_cat$predicted, d_cat$observed)

# or

library(caret)
cm <- confusionMatrix(d_cat$predicted, d_cat$observed, positive = "alpha")
cm$table
cm$overall
cm$byClass
```
