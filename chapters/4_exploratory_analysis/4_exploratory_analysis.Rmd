...
## Exercise 3: run the "mapunit comparison" report

You can create any R-based analysis in `soilReports` format. Another popular report is the `region2/mu-comparison` report. This report uses constant density sampling to extract numeric and categorical values from multiple raster data sources that overlap a set of polygons. In this example, we clip a small portion of SSURGO polygons from the CA630 soil survey area extent. 

We select a small set of mapunit symbols (5012,3046,7083,7085,7088) that occur within the clipping extent that have soil forming factors we expect to contrast with one another.
...
# download raster data, clip of SSURGO from an area of CA630, and sample script for
#  creating your own raster data clip
...
If you want, you can now set up the default `config.R` that is created by `copyReport()` to work with any of your own data. 

The ZIP file downloaded above includes a "sample" `config.R` file (called `new_config.R`). Run the line of code below to replace the default with the sample data.
...
Now, open report.Rmd in the mucomp folder and click the "Knit" button at the top of the source pane
