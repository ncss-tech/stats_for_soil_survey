...
r <- seq(-5, 5, 0.1)
d <- list(x = r, y = dnorm(r, sd = 2))

old.par <- par(no.readonly = TRUE)
par(mfrow = c(1, 2))
par(pty = "s")

plot(d, type = "l", main = "1 Standard Deviation \n (68% highlighted)", xlab = "")
...
plot(d, type = "l", main = "2 Standard Deviation \n (95% highlighted)", xlab = "")
...
r <- seq(-1, 2, 0.1)
plot(r, dunif(r), type = "l", main = "Uniform Distribution: Minimum = 0, Maximum = 1")
```
...
```{r, eval=FALSE}
# Install the soilReports package
...
report_path <- system.file(paste0('reports/', reportName), package='soilReports')

# Run the report
render(input = report_path, output_file = "C:/workspace/test.html")
...
## Exercise 2: run the pedon\_summary\_by\_taxonname.Rmd
...
# ))
# 
test <- read.csv("C:/workspace/github/stats_for_soil_survey/trunk/data/ca794_zonal.csv") # import the results
...
```{r}
# s <- spsample(ca794, n = 1000, type = "stratified") # take a stratified random sample
# 
...
load(file = "C:/workspace/github/stats_for_soil_survey/trunk/data/ch2_sample.Rdata")

summary(subset(test2, MUKEY == 2480977)) # examine summary for mukey 2480977
```
