...
Typo errors such as these are in old NASIS pedon data.

###<a id="freq")></a>4.1  Frequency distribution  

Now that we've checked for missing values and typos and made correcitons, we can graphically examine the sample data distribution of our integer and numeric data.
...
Next will review the `hist` function which plots a histogram.  
...
Compared to the histogram where the y-axis represents the number (i.e. frequencey) of observations, the y-axis for the density plot represents the proportion or probability of observing any given value.
...
# or using the summary function we could get the mean and median
