...
test <- merge(test, co[c("cokey", "slope_phase")], by = "cokey", all.x = TRUE)

test2 <- test
depths(test2) <- cokey ~ hzdept_r + hzdepb_r
test3 <- aqp::slice(test2, seq(0, 100, 2) ~ claytotal_r + om_r + caco3_r + slope_phase)
...
ggplot(test4, aes(x = hzdept_r, y = value)) +
  ...
  geom_smooth(aes(col = slope_phase), se = FALSE, lwd = 2) +
  xlim(100, 0) +
  xlab("depth (cm)") + ylab("clay (%)") +
  coord_flip() +
