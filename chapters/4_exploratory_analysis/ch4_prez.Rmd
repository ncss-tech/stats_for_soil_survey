...
crosby <- fetchSDA_component(WHERE = "muname LIKE 'Crosby%'")
attach(crosby)
c2 <- subsetProfiles(spc, s = "cokey == 14842603")
c_h <- horizons(c2)
names(c_h) <- gsub("total", "", names(c_h)) 

vars <- c("hzname", "clay_l", "clay_r", "clay_h", "texture")
knitr::kable(c_h[vars])
...
```{r}
with(h, table(genhz, texture_class, useNA = "ifany"))
...
co <- get_component_from_SDA(WHERE = "compname = 'Crosby'")
co2 <- subset(co, majcompflag == "Yes")
h  <- get_chorizon_from_SDA(WHERE = "compname = 'Crosby'")
test <- subset(h, cokey %in% co2$cokey)

# ordering of groups is handled by ggplot2
ggplot(test, aes(x = jitter(hzdept_r, 3), y = claytotal_r)) + 
  geom_line(aes(group = cokey), alpha = 0.2) + 
  geom_smooth(method = "loess") + 
  xlim(100, 0) + ylab("clay percent") + xlab("clay(%)") +
  coord_flip() +
  theme(aspect.ratio = 1) +
  ggtitle("Crosby Components")
