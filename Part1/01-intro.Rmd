...
-   `==` Equal To (NOTE: R uses a double equal sign)

-   `!=` Not Equal To

-   `<, >, <=, >=` Less than, greater than, less than or equal to, and greater than or equal

-   `&` Equivalent to `AND` must match both conditions

-   `|` Equivalent to `OR` must match at least one condition

-   `%in%` equivalent to `IN ()` in SQL (e.g. `mu$areasymbol %in% c("IN001", "IN111"`)

-   `grepl()` similar to `LIKE` in SQL (e.g. `grepl("IN%", mu$areasymbol)`)
