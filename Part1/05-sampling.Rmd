...
n <- 10
set.seed(123)
fs_strata <- strat_kmeans(rs, nStrata = k, iter = 10000)
set.seed(123)
fs_samp <- sample_strat(fs_strata, nSamp = n, allocation = "equal", method = "random")
...
# Compute weights
k_n <- table(value = fs_samp$strata) |> 
...
# create a RasterStack from SpatRaster in previous example
library(clhs)
...
# random sample with spatSample from terra package
sub_s <- spatSample(rs, size = 200, as.points = TRUE)

# convert to sf object and do clhs sampling
s <- clhs(st_as_sf(sub_s), size = 20, progress = FALSE, simple = FALSE)
```
...
# take a CSCS sample
fs_samp0 <- sample_nc(rs, nSamp = 20) 
fs_samp <- fs_samp0 |>
  terra::extract(rs, y = _) |> 
  select(elev, slope)
...
s <- rbind(
  ...
  data.frame(method = "cLHS", cs$sampled_data),
  data.frame(method = "CSCS", fs_samp[c("elev", "slope")])
)
