...
Below is a list of external resources for for 5 packages that provide different ways displaying spatial data graphically:
...
In this section we will introduce the `sf` package and interactive maps with `mapview`.
...
There is a simple conversion function for this in `sf`. For the single `sf` data.frame object class there are several corresponding `sp` classes depending on geometry type and whether they have an attribute table. All the `sp` class names start with the word "Spatial" e.g. "SpatialPolygonsDataFrame" has polygon geometry with associated attributes, whereas "SpatialPoints" has point geometry with no attributes.

```{r, eval=FALSE}
sf::as_Spatial(x)
```

In general you should avoid using the `sp` package classes and functions in new work, but you may encounter packages or workflows that require you to convert between them.
