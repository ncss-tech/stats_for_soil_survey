...
See the [Subsetting SoilProfileCollection Objects](https://ncss-tech.github.io/aqp/articles/Introduction-to-SoilProfileCollection-Objects.html#subsetting-soilprofilecollection-objects) reference for a more detailed explanation.
...
The `soilDB` package for R provides functions for accessing data stored in NASIS, KSSL, SDA, SoilWeb, SoilGrids, and other sources. Functions that use data from SoilWeb are documented in a new [vignette](https://ncss-tech.github.io/soilDB/articles/soilweb-data-functions.html).
...
For now we will start with the 'fetch' functions and others that will get you a large variety of data you can use for soil and ecological site analyses. There are many more functions available in soilDB, see https://ncss-tech.github.io/soilDB/reference/index.html for details.
...
# future TODO: limit to first ~10 items?
...
Note that logical operations on objects containing `NA` will evaluate to `NA`. If you plan to use a logical vector for subsetting, be sure to use `which()` to convert all instances of `TRUE` into their corresponding integer index.
...
plotSPC(subset1[1:10, ], label = 'taxsubgrp', max.depth = 60, cex.names = 0.8, cex.id = 0.66)
...
See the [Pair-Wise Distances by Generalized Horizon Labels](http://ncss-tech.github.io/AQP/aqp/genhz-distance-eval.html) demonstration, for an example of how GHL can be used to summarize "differences" between profiles within collection via numerical classification.
