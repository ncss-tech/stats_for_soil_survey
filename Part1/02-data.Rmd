...
-   Our confidence in observations typically weakens with the depth of the material described.
    ...
    -   How do we deal with it in pedon data?
        -   Use a cutoff depth, for example 100 cm, can be used to truncate observations
...
-   Description style differences
    -   Depth described, horizon designation usage styles
-   Legacy data vintage
    -   Data spanning multiple decades of standards
    -   Local, regional, and national variations in standards
...
The [*SoilProfileCollection*](http://ncss-tech.github.io/aqp/articles/Introduction-to-SoilProfileCollection-Objects.html) class (SPC) provided by the `aqp` package is a specialized structure for soil data analysis. It simplifies the process of working with collections of data associated with soil profiles, e.g., site-level, horizon-level, spatial, diagnostic horizons, and other metadata.
...
The *SoilProfileCollection* is an S4 R object. S4 objects have [slots](https://stat.ethz.ch/R-manual/R-patched/library/methods/html/slot.html). The main slots you need to know about are the slots for site-level and horizon-level data.

### *SoilProfileCollection* methods

Many familiar R functions are defined for the *SoilProfileCollection* object. Some are unique, and others operate like more common functions of vector and `data.frame` objects, such as `nrow()` (*"how many horizons?"*) or `length()` (*"how many sites/pedons?"*).

When you access the site data (with `site(<object>)`) or the horizon data (with `horizons(<object>)`) of a *SoilProfileCollection*, you get a `data.frame` object that you can use like any other in R.

#### Promoting a `data.frame` to *SoilProfileCollection*

The *SoilProfileCollection* object is a collection of 1-dimensional profile descriptions, like you might see on a Form 232, or of tabular data returned from laboratory.

The object is "horizon data forward" in that you start with the layers, and can add or create site-level attributes by normalization, joins, and calculations.

Most of the time if you are using your NASIS data, or an official database, there are defined ways of getting the data "into" an SPC. For example, `fetchOSD` returns a *SoilProfileCollection* that has been assembled from horizon and site level attributes parsed from the OSD text, Soil Classification database, and other sources.

In the pre-course, we had you set up the connection to your local NASIS instance to "fetch" data and have methods like `fetchNASIS` put things together for you.
...
The formula expresses the idea that a profile `id` has layers defined by set of `top` and `bottom` depths.
...
### Promoting to "Spatial" *SoilProfileCollection*
...
-   `[`: special SoilProfileCollection-specific "k-index" keywords
    -   `x[, , .FIRST]`: first horizon of every profile
    -   `x[, , .LAST]`: last horizon of every profile
    -   `x[, , .LAST, .BOTTOM]`: bottom depth of the last horizon of every profile

See the [Subsetting SoilProfileCollection Objects](https://ncss-tech.github.io/aqp/articles/Introduction-to-SoilProfileCollection-Objects.html#subsetting-soilprofilecollection-objects) reference for more details.
...
The *SoilProfileCollection* also has `[` ("single bracket"), but with a different interpretation than we have with the `[i, j]` indexing of `data.frame` objects.

-   In a `data.frame` you have `object[row, column, drop=TRUE]`; the result is a `data.frame` (or a vector with default `drop=TRUE`).

-   In a *SoilProfileCollection* you have `object[site, horizon]`; the result is always a *SoilProfileCollection*.
...
## Exercise 1: Assemble a *SoilProfileCollection* from several CSV files {.unnumbered}
...
The `soilDB` package for R provides functions for accessing data stored in NASIS, KSSL, Soil Data Access (SDA), SoilWeb, SoilGrids, and other sources. Functions that use data from SDA are documented in a [vignette](https://ncss-tech.github.io/soilDB/articles/sda.html), also functions that use data from SoilWeb are documented in a [vignette](https://ncss-tech.github.io/soilDB/articles/soilweb-data-functions.html).

These high-level 'fetch' functions bundle lower-level 'get' functions that access internal database interfaces to NASIS and other data sources. The ODBC connection to NASIS that you set up during the pre-course is an one example of a database interface.

Basic data checks are run within the 'fetch' functions. These checks ensure the basic integrity of the data as it is queried and moved from its existing structure into an SPC or other final object. There are times when it is useful to use the lower-level `get` functions individually. They generally return single `data.frame` or a `list` of `data.frame`.

You can set up scripts to make custom queries against these or other sources on your own. There is an example at the end of this chapter.

For now we will start with the 'fetch' functions and others that will get you a variety of data you can use for soil and ecological site analyses.

There are many more functions available in soilDB, see <https://ncss-tech.github.io/soilDB/reference/index.html> for the full list.
...
-   [**SDA_query()**](http://ncss-tech.github.io/soilDB/reference/SDA_query.html)
    ...
    -   Submits queries to the Soil Data Access system.
        -   [Soil Data Access Tutorial](https://ncss-tech.github.io/soilDB/articles/sda.htm)
...
-   [**fetchOSD()**](http://ncss-tech.github.io/soilDB/reference/fetchOSD.html)
    ...
    -   Returns soil series summaries (as `SoilProfileCollection`) derived from Official Series Descriptions, SSURGO, and other data sources.
        -   [Querying Soil Series Data](http://ncss-tech.github.io/AQP/soilDB/soil-series-query-functions.html)
        -   [Data Sources](https://ncss-tech.github.io/soilDB/articles/soilweb-data-functions.html)
...
Instead, it pulls essential / commonly used pedon and horizon data, and makes several assumptions about those data. A lot of the nested complexity of NASIS is simplified in the resulting object.
...
For example, many-to-one relationships are "flattened" wherever possible. This "flattening" aggregates the data from various tables into one "site" or "horizon" record.
...
With `siteNames()` and `horizonNames()` we can view the names for each slot respectively in the `gopheridge` object.
...
-   [*SoilProfileCollection* Reference](http://ncss-tech.github.io/aqp/articles/Introduction-to-SoilProfileCollection-Objects.html)
...
For `gopheridge` the majority of pedons are *Loamy-skeletal Ultic Haploxeralfs*, which generally aligns with the series concept.
...
Table objects can be subset and re-ordered using `[`-style indexing. Sometimes it is helpful to shuffle rows and columns so that the most frequent combinations are in the "upper left" corner of the table.

```{r, eval=TRUE}
# cross-tabulation: hzname x texture class
tab <- table(pedons$hzname, pedons$texcl)

# row ordering index
o.row <- order(rowSums(tab), decreasing = TRUE)

# column ordering index
o.col <- order(colSums(tab), decreasing = TRUE)

# re-order cells of table
tab <- tab[o.row, o.col]
...
Missing values are encoded in R as `NA`. By default, the `table()` function will ignore `NA`. The `useNA` argument can conditionally (`useNA = 'ifany'`) or always print counts of `NA` (`useNA = 'always'`).

Use `is.na()` to return a logical value that identifies elements of an object that are missing data. Use `which()` to convert a logical vector into an index of elements that are `TRUE`.

```{r, eval=TRUE}
# NA in moist hue
table(pedons$m_hue, useNA = 'ifany')

# proportion of values that are not NA
prop.table(table(!is.na(pedons$m_hue)))
```

### Logical Operators {#logical-operators-detail}

Logical values in R encode one of three constant values: `TRUE`, `FALSE`, or missing (`NA`).

[Logical operators](https://en.wikipedia.org/wiki/Logical_connective) are functions that compare inputs using the rules of [Boolean Algebra](https://en.wikipedia.org/wiki/Boolean_algebra) and return a logical value or vector. Some functions such as the test for equality (`==`) or inequality (`!=`) accept many possible data types (numeric, character) and return a vector of logical values.

Logical vectors (e.g. `c(TRUE, FALSE, FALSE, TRUE)`) are a way to express complex subsetting tasks such as "find all records that represent either alfisols OR inceptisols, but not records that contain the 'duri-' formative element":

``` r
order_lgl <- pedons$taxorder %in% c("alfisols", "inceptisols")
subgroup_lgl <- !grepl("duri", pedons$taxsubgrp)
combined_lgl <- order_lgl & subgroup_lgl
subset(pedons, combined_lgl)
```

Logical operations can be visualized as [Venn diagrams](https://en.wikipedia.org/wiki/Venn_diagram). Some of the more common logical operators are demonstrated in the following figure. Logical operators can be combined using parenthesis to describe complex subsetting criteria. ![Commonly used logical operators, described using Venn diagrams.](static-figures/logical-operator-examples.png)

Common operators and functions that return logical vectors include:

-   `A == B`: "A is equal to B", note the use of double-equal sign

-   `A != B`: "A is not equal to B", note that `!` can be used to invert any logical vector

-   `<`, `>`, `<=`, `>=`: inequality operators

-   `grepl(pattern, x)`: text `pattern` found in `x`, similar to SQL `LIKE` (e.g. `areasymbol LIKE "CA%"`), [REGEX Resources in Chapter 2](#regex-resources)

Vectorized logical operators:

-   `A & B`: `&` is the ["AND" operator](https://en.wikipedia.org/wiki/Logical_conjunction), "intersection of A and B"

-   `A | B`: `|` is the ["OR" operator](https://en.wikipedia.org/wiki/Logical_disjunction), "union of A and B"

-   `xor(A, B)`: [exclusive OR](https://en.wikipedia.org/wiki/Exclusive_or), "A or B, but not both"

-   `A %in% B`: equivalent to SQL `IN` operator (e.g. `areasymbol IN ("IN001", "IN111")`)

Logical aggregate functions (return a single value):

-   `all(A)`: test that all elements of `A` are `TRUE`

-   `any(A)`: test that one or more elements of `A` are `TRUE`

Left-to-right logical operators:

-   `A && B`: logical AND, if A is `FALSE` B is not evaluated, if A or B are length \> 1, only the first is evaluated

-   `A || B`: logical OR, if A is `TRUE` B is not evaluated, if A or B are length \> 1, only the first is evaluated

Note: logical operations on objects containing `NA` will evaluate to `NA`. If you plan to use a logical vector for subsetting, either use `which()` to convert all instances of `TRUE` into their corresponding integer index, or use a function that implicitly treats `NA` as `FALSE`, such as `subset()` or `dplyr::filter()`
...
Profile sketches are a convenient way to check the results of manipulating a `SoilProfileCollection`.

```{r, eval=TRUE, fig.width=8, fig.height=4.5}
# adjust margins
par(mar = c(0.5, 0, 0.5, 2))

# note the first 10 profiles of subset1 are selected
plotSPC(
  subset1[1:10, ],
  label = 'taxsubgrp',
  max.depth = 60,
...
title('Pedons with the word "typic" at subgroup-level of Soil Taxonomy', line = -1.5)
```

#### Resources for Regular Expressions {#regex-resources}
...
subset2 <- subset(pedons,
                  grepl("alfs", taxsubgrp) & obsdate < as.POSIXlt("2010-01-01"))
...
## Exercise 2: O Horizon Thickness {.unnumbered}
...
We will start to examine this dataset graphically, and then we will use some functions defined for the *SoilProfileCollection* to extract information from the data.

Save the code you use in an R script, add answers as comments, and send to your mentor.

First, we will load the sample data:

```{r, eval=TRUE, fig.width=8, fig.height=4}
# read an RDA file into R session
# this creates an object `ohz`
load(url("https://github.com/ncss-tech/stats_for_soil_survey/raw/master/data/book/02/ohz.rda"))

par(mar = c(0, 0, 0, 2))
plotSPC(ohz, cex.names = 0.75)
```
...
We will use the `site()` LEFT JOIN method to merge the calculated thickness into the *SoilProfileCollection*.
...
```{r, eval=TRUE, fig.width=8, fig.height=4}
par(mar = c(0, 0, 0, 2))
plotSPC(ohz, plot.order = order(ohz$O_thickness), cex.names = 0.75)

# horizontal lines at 20cm and 40cm
abline(h = c(20, 40), lty = 2, col = 'firebrick')
```

Using the `ohz` object answer the following questions. Use techniques from the previous sections. Check your logic by comparing to the profile sketch.
...
2.  Tabulate horizon designations (`ohz$name`) using `table()`. How many of each horizon designation are present?
...
5.  Filter the `ohz` object using `aqp::subset()` to select pedons where `O_thickness` column is greater than or equal to `40`. This is the Histosols order thickness requirement. How many pedons are Histosols?
...
![A hand drawn sketch of soil profiles, basic morphology, and notes related to the correlation of major classes of genetic horizon.](static-figures/genhz-sketch.png)
...
![A demonstration of generalized horizon labels applied to pedons of the Clarksville soil series.](static-figures/GHL-example.png)
...
Generalized horizon labels are an aggregation strategy. Here is a basic outline of the process:
  ...
  2. Assign GHL to each horizon using whatever information is available for grouping horizons. This micro-correlation of horizon designations will often require slightly different rules for each dataset and group. Careful inspection of horizon designation and observed properties is critical.
...
[Exercise 4](#p1-ch2-ex4) in this chapter will let you work through the process of summarizing data using GHLs with your own data. But first we continue with tools to inspect your data retrieved from NASIS with soilDB. It is very important that basic QC/QA be performed on your data before proceeding with any statistical summaries.
...
When `fetchNASIS(..., rmHzErrors = TRUE)` any horizons that were omitted from the *SoilProfileCollection* will be stored in the `bad.horizons` variable in the `soilDB.env` environment.
...
Once you have created a subset *SoilProfileCollection*, you can manually inspect the data in that set to identify areas where there are issues. Ideally you would fix these in NASIS. If you don't have the ability to fix them in NASIS, you might write some R code into your analysis script to apply patches to missing or invalid data before proceeding with summaries.

## Extended Data Functions

Additional data related to both site and horizon information can be fetched using the [`get_extended_data_from_NASIS()`](http://ncss-tech.github.io/soilDB/reference/get_extended_data_from_NASIS.html) function. This function returns a named *list* object with several tables that `fetchNASIS()` draws from.
...
```{=html}
<!-- TODO: add basic description / examples for:
...
## Exercise 3: Diagnostic Horizons in Your Own Data {.unnumbered}
...
## Exercise 4: Generalized Horizons with Loafercreek {#p1-ch2-ex4}
...
-   [Exercise: Generalized Horizon Labels with Loafercreek](http://ncss-tech.github.io/stats_for_soil_survey/exercises/genhz_homework.html)

For the exercise during class we will give you some time to read over the materials and apply the process to the sample `loafercreek` data.

The code to apply the process to `loafercreek` is given. We will ask you to apply the process to your own data, adjusting your generalized horizon patterns as needed for your local soils. You will send a table and profile plots to your mentor when you are done.

This may take more time than we have during class itself, so you should follow up as needed with your mentor to complete the exercise.

Chapter 3 Exploratory Data Analysis will get deeper into some of the topics that are referenced in the `loafercreek` code, such as summary statistics on grouped data.

See the [Pair-Wise Distances by Generalized Horizon Labels](http://ncss-tech.github.io/AQP/aqp/genhz-distance-eval.html) demonstration, for an example of how GHL can be used to summarize "differences" between profiles via numerical classification.
