---
title: Chapter 5 - Generalized Linear Models (GLM)
...
wv$spod <- ifelse(wv$spodint <= 1, 0, 1)

```
```{r, echo=TRUE}
spod.glm <- glm(spod ~ dem10m + eastness + northness + maxent, family = binomial(link = "logit"), data = wv)
print(spod.glm)
```
...
```{r acc1, echo=TRUE}
library(rms)
lrm(spod ~ dem10m + eastness + northness + maxent, data = wv)
```
...
## Interpreting the Coefficients - Partial-Residual Plots

```{r coef2}
car::crPlots(spod.glm)
```
...
```{r}
car::residualPlots(spod.glm, main = "Residual Plots")
```
