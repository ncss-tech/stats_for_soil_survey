...
test3 |>
  group_by(component) |>
  summarize(
    p = srvyr::survey_prop(vartype = "ci", level = 0.9, method = "logit", conf.level = 0.9)
  )
table(sample$component)
DescTools::BinomCI(3, 6, method = "logit", conf.level = 0.9)
...
table(samp$component) |> 
  prop.table() |> 
  round(2)
samp_vals <- sapply(1:1000, function(i) {
...
# DescTools
# this is approximate since it doesn't account for the true sample size
# must used scale confusion matrix
cm_map_CI <- cm_scaled |> 
