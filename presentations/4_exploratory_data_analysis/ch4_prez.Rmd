...
#   prop.table() |> 
#   round(2)
samp_vals6 <- sapply(1:100, function(i) {
...
strata <- fcm_r2[[8]]
names(strata) <- "strata"
test <- sample_strat(strata, 10, allocation = "equal", method = "random")
test <- extract(fcm_r2, st_coordinates(test))

# terra
strata <- fcm_r2[[8]]
test_samp <- spatSample(strata, size = 10, method = "stratified", as.points = TRUE)
test_samp <- extract(fcm_r2, test_samp)

table(test_samp$k_0p025acres)
table(test_samp$k_smooth5_1p4acres)

test_pop <- as.data.frame(fcm_r2)
```
...
### accuracy
```{r}
test_samp2 <- merge(test_samp, tab, by.x = "k_0p025acres", by.y = "value")
names(test_samp2)[c(1, 10)] <- c("obs", "pred")

cm <- xtabs(wk_e ~ pred + obs, data = test_samp2)
cm2 <- aqp::tauW(cm)
cm
cbind(round(cm, 2), ua = cm2$user.naive) |> round(2) |> rbind(pa = c(cm2$prod.naive, cm2$overall.naive) |> round(2))
...
cm2
mapac::aa_card(x = test$k_0p025acres, m = test$k_smooth5_1p4acres)
