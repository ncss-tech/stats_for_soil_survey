...
gridExtra::grid.arrange(gg_cn, gg_lf, gg_a, gg_b, top = "Map Unit A-B")
...
samp_vals <- sapply(1:100, function(i) {
  sample(sample$component, size = n - ceiling(n * 0.1), prob = 1 / sample$w) |> 
    table() |> 
    prop.table()
  }, 
  simplify = FALSE
...
samp_lrh <- apply(samp_vals, MARGIN = 2, FUN = function(x) quantile(x, p = c(0.1, 0.5, 0.9))) |> 
  t() |> 
  round(2)
samp_lrh  


n <- ceiling(nrow(grd) * 1)
samp <- st_sample(grd, size = n) |> 
  st_coordinates() |>
  terra::extract(x = vect(grd), y = _)
table(samp$component) |> 
  ...
  round(2)
samp_vals <- sapply(1:100, function(i) {
  sample(samp$component, n - ceiling(n * 0.1)) |> 
    table() |> 
    prop.table()
  }, 
  simplify = FALSE
...
pop_lrh <- grd |> 
  ...
  round(2) |>
  t()
pop_lrh2 <- cbind(ind = row.names(pop_lrh), pop_lrh) |>
  as.data.frame()
names(pop_lrh2) <- c("ind", "L", "RV", "H")
pop_lrh2 <- pop_lrh |> t() |> as.data.frame() |>
  stack()
...
gg_A <- grd_lo |>
  ...
  geom_vline(xintercept = pop_lrh2$values[pop_lrh2$ind == "A"][1], linetype = "dashed") +
  geom_vline(xintercept = pop_lrh2$values[pop_lrh2$ind == "A"][3], linetype = "dashed") +
  geom_vline(xintercept = pop_lrh2$values[pop_lrh2$ind == "A"][2]) +
...
gg_B <- grd_lo |>
  ...
  ggplot(aes(x = values)) +
  geom_histogram(breaks = seq(0, 1, 0.1)) +
  geom_vline(xintercept = pop_lrh2$values[pop_lrh2$ind == "B"][1], linetype = "dashed") +
  geom_vline(xintercept = pop_lrh2$values[pop_lrh2$ind == "B"][3], linetype = "dashed") +
  geom_vline(xintercept = pop_lrh2$values[pop_lrh2$ind == "B"][2]) +
  # facet_wrap(~ ind, ncol = 1) +
  ggtitle("Component B") +
  xlab("Probability")
gg_AB_pop <- gridExtra::grid.arrange(gg_A, gg_B, top = "Population", nrow = 1)
...
gg_As <- samp_lo |>
  ...
  ggplot(aes(x = values)) +
  geom_histogram() +
  geom_vline(xintercept = samp_lrh[1, 1], linetype = "dashed") +
  geom_vline(xintercept = samp_lrh[1, 3], linetype = "dashed") +
  geom_vline(xintercept = samp_lrh[1, 2]) +
  # facet_wrap(~ ind, ncol = 1) +
  ggtitle("Component A") +
...
gg_Bs <- samp_lo |>
  ...
  ggplot(aes(x = values)) +
  geom_histogram() +
  geom_vline(xintercept = samp_lrh[2, 1], linetype = "dashed") +
  geom_vline(xintercept = samp_lrh[2, 3], linetype = "dashed") +
  geom_vline(xintercept = samp_lrh[2, 2]) +
  # facet_wrap(~ ind, ncol = 1) +
  ggtitle("Component B") +
  xlim(0, 1) + xlab("Probability")
gg_AB_samp <- gridExtra::grid.arrange(gg_As, gg_Bs, top = "Hardened Sample", nrow = 1)
