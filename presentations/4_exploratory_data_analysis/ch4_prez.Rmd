...
prop.table(cm, margin = 1) |> round(2)
cm2 <- aqp::tauW(cm/70)
round(cm/70, 1)
cbind(
  as.data.frame.matrix(prop.table(cm, 1)) |> round(2), ua = cm2$user.naive) |> 
  round(2) |> 
  rbind(pa = c(cm2$prod.naive, cm2$overall.naive) |> round(2))
table(pred = population$map, obs = population$obs) |> prop.table(1) |> round(2)
...
# DescTools
cm_p <- xtabs(wk_e ~ pred + obs, data = test_samp21)
cm_p |> prop.table(margin = 1) |> round(2)
DescTools::MultinomCI(cm_p[1, ])
DescTools::MultinomCI(cm_p[1, ], method = "wilson")
DescTools::MultinomCI(cm_p[1, ], method = "goodman")


# bootstrap
test <- by(test_samp21, test_samp21$pred, function(x){
