...
ggplot(test, aes(x = sample, y = index)) +
  ...
  ggtitle("Example of the sample errors")
...
strat_kmeans(volcano_r[[idx_uf]], nStrata = 7) |> freq() |> transform(p = round(count / sum(count), 2))
strat_kmeans(volcano_r[[idx_f]], nStrata = 7) |> 
  freq() |> 
  dplyr::mutate(
    p  = count / sum(count), 
    wp = count / (70 * p),
...
test <- spatSample(k_smooth5, size = 5, as.points = TRUE)


test <- spatSample(fcm_r2, size = 30)

cm <- table(pred = fcm_r2$k_smooth5_1p4acres |> values(), obs = fcm_r2$k_0p025acres |> values())
cm <- table(pred = test$k_smooth5_1p4acres, obs = test$k_0p025acres)
