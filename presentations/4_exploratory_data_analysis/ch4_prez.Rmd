...
grd <- within(grd, {
  comppct_r <- ifelse(
    idx, 
    rlogis(sum(idx),   boot::logit(0.75)) |> boot::inv.logit(),
    rlogis(sum(! idx), boot::logit(0.25)) |> boot::inv.logit()
    )
  A <- comppct_r
  B <- 1 - comppct_r
...
ggplot() +
  ...
  geom_sf(
    data = mutate(sample), #, component = component), 
...
tb_lf <- xtabs(~ landform + comp, data = grd) |> 
  prop.table()
tb_lf |>
...
grd$mu <- "test"
grd |> 
  select(compname, w, A, B, mu) |>
  # pivot_longer(cols = c(A, B)) |>
  group_by(mu) |>
  reframe(across(A:B, ~DescTools::Quantile(x = .x, weights = w, probs = c(0.1, 0.5, 0.9)) |> t() |> round(2)))

grd |> 
  select(compname, w, A, B, mu) |>
  pivot_longer(cols = c(A, B)) |>
  group_by(mu, compname, name) |>
  reframe(across(value, ~DescTools::Quantile(x = .x, weights = w, probs = c(0.1, 0.5, 0.9)) |> t()))


table(grd$compname)
mean(grd$A)
quantile(grd$A, p = seq(0, 1, 0.1))
