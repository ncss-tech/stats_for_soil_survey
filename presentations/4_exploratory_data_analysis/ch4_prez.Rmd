...
test_samp1 <- extract(fcm_r2, st_coordinates(test_samp1))
...
### accuracy
```{r}
# test_samp12 <- merge(test_samp1, tab, by.x = "k_smooth5_1p4acres", by.y = "value")
# names(test_samp12)[c(1, 9)] <- c("pred", "obs")
# xtabs(wk_e ~ pred + obs, data = test_samp12)

sample <- merge(samp, tab, by.x = "map", by.y = "value")
# sample(test_samp21)[c(1, 10)] <- c("obs", "pred")
saveRDS(
  list(
    sample = sample, 
  ...
  file = "C:/workspace2/github/ncss-tech/stats_for_soil_survey/data/exploratory_data/volcano.rds"
  )


# confusion matrix
cm <- xtabs(wk_equal ~ map + obs, data = sample)
cm_scaled <- round(cm/70, 2)
cm_scaled


# map unit percentages
comppct_r <- prop.table(cm, margin = 1) |> round(2)*100
comppct_r
...
cbind(
  as.data.frame.matrix(cm_scaled), ua = cm2$user.naive) |> 
  round(2) |> 
  rbind(pa = c(cm2$prod.naive, cm2$overall.naive) |> round(2))
...
ds |>
  group_by(map, obs) |>
  summarize(p = survey_prop(vartype = "ci", level = 0.8)) |>
  filter(map == "Alpha")
...
cm_map_CI <- cm_scaled |> 
  apply(MARGIN = 1, FUN = function(x) {
    DescTools::MultinomCI(x, conf.level = 0.8, method = "wilson")
    }, 
    simplify = FALSE
    )
cm_map_CI$Alpha
