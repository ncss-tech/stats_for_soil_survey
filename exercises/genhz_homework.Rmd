...
You have a collection of pedons correlated to a soil series or component. You would would like to compute the _Range in Characteristics_ (“low-rv-high” values; RIC) for these pedons.
...
Here we have `r length(unique(pedons$hzname))` different horizon designations. We would not would attempt to make separate ranges for each unique group, especially with such a small amount of data in some of the groups.
