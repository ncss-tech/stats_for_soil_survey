## Assemble a SoilProfileCollection from CSV Data
...
## original Sierra Transect (central Sierra, granite) data from CSV ----
# horizon + site data
granite <- read.csv(
  'https://github.com/ncss-tech/aqp/raw/master/misc/example-data/sierra-transect/dahlgren-granitics.csv'
)

## paired transect to the north formation transect from CSV ----
# horizon data
andesite <- read.csv(
  'https://github.com/ncss-tech/aqp/raw/master/misc/example-data/sierra-transect/rasmussen-andesitic-lahar.csv'
)

# site data
andesite.site <- read.csv(
  'https://github.com/ncss-tech/aqp/raw/master/misc/example-data/sierra-transect/rasmussen-andesitic-lahar-site.csv'
)


# check that these are data.frame objects
class(granite)
...
# upgrade to a SoilProfileCollection ----
...
# fancy plot
# adjust plotting region or plot zoom window as needed
par(mar = c(0, 0, 0, 0))
plotSPC(g, name.style = 'center-center', depth.axis = FALSE, hz.depths = TRUE, hz.depths.offset = 0.05, cex.names = 0.66, width = 0.3, shrink = TRUE)
...
gis.data <- read.csv(
  'https://github.com/ncss-tech/aqp/raw/master/misc/example-data/sierra-transect/transect-GIS-data.csv'
)

# note "id" column
head(gis.data)

# merge-in external data via common ID ----
# this is an implicit left join
site(g) <- gis.data

# profile sketches, ordered according to elevation and effective PPT
par(mar = c(0, 0, 0, 1))
plotSPC(g, width = 0.3, plot.order = order(g$elev))
plotSPC(g, width = 0.3, plot.order = order(g$effective.ppt_800))

# color horizons based on clay fraction
par(mar = c(0, 0, 3, 1))
...
# investigate Fe oxide production vs. climate ----
