...
In order to create repeatable results, the following examples use the `set.seed()` function to control randomness. This can be a useful trick when it is necessary to generate the same output each time a report or analysis is run. However, one should not use `set.seed()` when a truly random outcome is required.


### Planned additions

  * evaluation of full vs. sampled data-space
...
plot(r.lab, col = hcl.colors(25, palette = 'mako'), axes = FALSE)
```
...
plot(H, col = hcl.colors(25), axes = FALSE, main = 'Shannon Entropy (log base 2)')
```
...
```{r fig.width=8, fig.height=5.5, dev = 'jpeg', fig.alt='Images of nMDS scores, estimated at each pixel of the example image.', fig.cap='Images of nMDS scores, estimated at each pixel of the example image.'}
# colors
mds1.cols <- colorRampPalette(c('green', 'red'), space = 'Lab')(25)
mds2.cols <- rev(grey.colors(25))

# maps
par(mfcol = c(1, 2))
plot(p.mds1, col = mds1.cols, axes = FALSE, main = 'nMDS Axis 1', mar = c(2, 1, 2, 3))
mtext(text = '"green \U2192 red"', side = 1, line = 0.5)
