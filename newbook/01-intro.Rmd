...
2. Load the legend table for `mlraoffice = "%"` and `areasymbol = "WI%` using `get_legend_from_NASISWebReport()`.
3. Filter your legend table for rows where the `ssastatus == "Update neeed"`.
4. Load the mapunit table, using `get_mapunit_from_NASISWebReport()`, for areasymbols identified in step 3.
4. Calculate the acreage of hydric soils for each map unit by multiplying `muacres * pct_hydric`.
5. Aggregate the total acreage of hydric soils each soil survey area using `group_by` and `summarize`.
6. Join your aggregated mapunit table to the legend table using a left join.
