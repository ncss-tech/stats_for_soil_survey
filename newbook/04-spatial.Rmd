...
**NOTE:** <strike>currently, there is no way to read raster data from ESRI "file" geodatabases.</strike> except for this: https://github.com/r-barnes/ArcRasterRescue, which requires ability to compile C code with `CMAKE` (non-government computer). Vector data (and associated attribute tables) can be accessed.
...
If you have a _.shp_ file, you can specify the whole path, including the file extension in the `dsn` argument.
...
addFeatures(m, st_as_sf(redding), color='black', fillColor='red', weight=1)
```
...
system.time({ ex <- exact_extract(r, s) })

# ex is a list(), with data.frame [value, coverage_fraction]
# for each polygon in s, we only have one, so we don't need this

# combine all list elements into single data.frame (row-wise)
# ex.all <- do.call('rbind', ex)

# or, just take the first with MULTIPOLYGON
ex.all <- ex[[1]]

# simple summary
densityplot(ex.all$value, plot.points=FALSE, bw = 2, lwd = 2,
            col = 'RoyalBlue', ylab = 'Density', from = 0, to = 400,
            xlab = 'Frost-Free Days (50% chance)\n800m PRISM Data (1981-2010)', 
            main = 'FFD Estimate for Extent of San Joaquin Series')
```
 ...
 * _EPSG_: `4269` / _PROJ.4_:`+proj=longlat +datum=NAD83` - geographic, NAD83 datum
 * _EPSG_: `4267` / _PROJ.4_:`+proj=longlat +datum=NAD27` - geographic, NAD27 datum
 * _EPSG_: `26910` / _PROJ.4_:`+proj=utm +zone=10 +datum=NAD83` - projected (UTM zone 10), NAD83 datum
 * _EPSG_: `6350` / _PROJ.4_: `+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=23.0 +lon_0=-96 +x_0=0 +y_0=0 +datum=NAD83 +units=m +no_defs` - Albers Equal Area CONUS (Used for gSSURGO)
...
st_crs(s) <- sf::st_crs(4326)
```

Transformation of points, lines, and polygons with `sf` just requires that origin be defined in the object that is the argument `x`, and target CRS defined in `crs` argument as an integer, or output of `st_crs()`.
...
Equivalent EPSG, OGC and PROJ.4 can be set or get using `proj4string<-`/`proj4string` and either a `sp` `CRS` object or a PROJ.4 string for `Spatial` objects.

```{r}
# s is a Spatial* object
s <- as_Spatial(s) 
...
The first method ensures that all data are included in the analysis, however, processing is very slow and the results may not fit into memory. 
...
If you haven't yet done so, please setup the sample data sets for this module. It will not take too long to download.
...
unzip(file.path(ch2b.data.path, 'chapter-2b-PRISM.zip'), exdir = ch2b.data.path, overwrite = TRUE)
```
...
We will be using polygons associated with MLRAs 15 and 18 as part of this demonstration. Import these data now with `readOGR()`; recall the somewhat strange syntax. You will need the data and `RasterStack` object `rs` we created in the examples above.
...
Consider something more modern than Hmisc. There are so many more generalizable options. -AGB
