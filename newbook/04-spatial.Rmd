...
x <- sf::read_sf(x, dsn = 'E:/gis_data/ca630/pedon_locations.shp')
```
...
Processing of raster data in memory is always faster than processing on disk, as long as there is sufficient memory.With the `raster` package, the initial file/disk-based reference can be converted to an in-memory `RasterLayer` with the `readAll()` function. You can achieve a similar effect in `terra` by doing `values(object) <- values(object)`, but bypassing the heuristics for loading raster data into memory is not recommended.
...
* `wkt(r)` (preferred) OR `crs(r)` OR `proj4string(r)`: get / set the coordinate reference system
...
Commonly used raster `datatypes` include: "unsigned integer", "signed integer", and "floating point" of variable precision.
...
download.file(
  ...
  file.path(ch4.data.path, 'chapter-2b-mu-polygons.zip')
)
...
download.file(
  ...
  file.path(ch4.data.path, 'chapter-2b-PRISM.zip')
)
...
Import these data  with `terra::vect()`.
...
We will load the sample raster data (PRISM derived) using `terra::rast()`
...
ppt_eff <- raster::raster(file.path(ch4.data.path, 'effective_preciptitation.tif'))
```
...
spatSample(rast(maat), size = 10)

# sampling RasterStack
spatSample(rast(maat), size = 10)
```

Sampling and extraction with `terra`, result is a `SpatVector` object.

```{r fig.width=8, fig.height=5}
par(mfcol = c(1, 2), mar = c(1, 1, 3, 1))

# regular sampling + extraction of raster values
x.regular <-  spatSample(rast(maat),
                         method = "regular",
                         size = 100,
                         as.points = TRUE)
x.regular
...
plot(st_geometry(mlra), main = 'MLRA 15 and 18')
box()
...
res2 <- dplyr::select(res, pedon_key, MAAT, MAP, FFD, GDD, rain_fraction, effective_preciptitation)
...
new.order <- order(auburn$effective_preciptitation)
...
axis(side = 1,
     at = 1:length(auburn),
     labels = round(auburn$effective_preciptitation[new.order]),
     cex.axis = 0.75)
...
e <- extract(rs, sf::as_Spatial(hexagonal), df = TRUE)
e.pts <- extract(rs, sf::as_Spatial(amador.pts), df = TRUE)
...
m <- tidyr::pivot_longer(s.df, cols = c(MAAT, MAP, FFD, GDD, rain_fraction, effective_preciptitation))
