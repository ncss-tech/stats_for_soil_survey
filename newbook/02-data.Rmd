...
The `SoilProfileCollection` also has `[` (i.e. "single bracket method"), but with a different interpretation from the `[i, j]` indexing of `data.frame` objects. 

 - In a `data.frame` you have `object[row, column, drop=TRUE]`; the result is a `data.frame` (or a vector with default `drop=FALSE`). 
...
All slots in the collection have a relationship to the "main" profile (i) index--so when you remove profiles, all associated records (e.g. spatial, diagnostics, horizons, etc.) in the object are removed.

Similarly, when all the horizons in a profile are removed (e.g. you request the 6th j-index from a profile that has only 5), the entire corresponding profile (site index) is removed.
...
- **url =** string specifying the (temporary) URL for the NASIS `pedon_report` output
