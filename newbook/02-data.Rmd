...
The aqp and soilDB packages were created just for this! soilDB returns tabular data, spatial data objects and SoilProfileCollection(SPC) objects! SoilProfileCollection objects are defined in the aqp (Algorithms for Quantitative Pedology) package as a handy abstraction for working with commonly used soils data.
...
The `SoilProfileCollection` is an S4 R object. S4 objects have slots (strictly defined "containers" within the S4 container). Of primary importance, are the slots for site-level and horizon-level data.
...
The base R functions for accessing and setting `data.frame` columns by name such as `$` and `[[` work for `SoilProfileCollection` objects, too. Review: 
...
##### Analogue to and Extention of Row Access: `[`

The `SoilProfileCollection` also has `[` (i.e. "single bracket method"), but with a different interpretation from the `[i, j]` indexing in `data.frame`. 

 - In a `data.frame` you have `object[row, column, drop=TRUE]`; the result is a `data.frame` (or a vector with default `drop`). 
...
# i-index: 2 profiles, 6 horizons
sp4[1:2, ] # first two profiles, all horizons

# j-index: 10 profiles; 20 horizons
sp4[, 1:2] # first two horizons of each profile 
```

TODO -- needs clarification -- Everything has gets subset simultaneously -- sites, horizons, spatial data, diagnostics, etc. in one command.

```{r, eval=TRUE}
# First profile, first two horizons
horizons(sp4[1, 1:2])
```
...
Basic data checks are run within `fetch` functions. These checks ensure the basic integrity of the data as it is queried and moved from its existing structure into an SPC. There are time with it is useful to use the lower-level `get` functions individually.  They generally return single `data.frame` or `list` of `data.frame`. 
...
```{r structure_diagram_a, purl=FALSE, echo=FALSE, warning=FALSE}
...
- **soilColorState = 'moist' or 'dry'** 
    - Select dry or moist colors to be converted and placed into a horizon-level attribute called `soil_color`. Moist and dry colors are also stored in `moist_soil_color` and `dry_soil_color`.
...
pedons <- gopheridge 
# NOTE: you can use fetchNASIS to load your own data, doing so would you would run it this way
# pedons <- fetchNASIS()
...
fetchNASIS does a lot of the hard work for you by pulling out a variety of child tables and joining them to site/horizon tables in order build a `SoilProfileCollection` object that has many of the NASIS attributes.

Analysis of Site Coordinate Data:
...
Quality control on fragments, horizon data, site data, etc.
...
With the default `rmHzErrors = TRUE` most of the profiles with ochric epipedons are _removed_ -- this is something you need to be aware of and inspect! 
```{r, eval=TRUE}
example.data.dir <- "C:/workspace2"
example.data.path <- file.path(example.data.dir, "mt647.zip")

if(!dir.exists(example.data.dir))
  dir.create(example.data.dir, recursive = TRUE)

download.file("http://ncss-tech.github.io/stats_for_soil_survey/data/book/02/mt647.zip", 
...
table(site(mt647)$ochric.epipedon, useNA = "ifany")

# get just the profiles with ochric.epipedon == TRUE
subset(mt647, ochric.epipedon)

# these are the mt663 bad.pedon.ids
get('bad.pedon.ids', envir = soilDB.env)

# the bad pedons are in f with rmHzErrors = FALSE
f$pedon_id %in% get('bad.pedon.ids', envir = soilDB.env)
```

The `aqp` packages has several functions that do logic checks on SoilProfileCollections. The main method is `checkHzDepthLogic` which returns a data.frame containing the results of running four logic tests on the horizon data from each profile.

Checks for:
 - bottom depths less than top depth / bad top depth order (`"depthLogic"`)
 - bottom depths equal to top depth (`"sameDepth"`)
...
All these "extended" data are not brought into a `SoilProfileCollection` _automatically_ because these data elements are related to the site or horizon data in one-to-many relationships.
...
When you ran the sample data download code, that code is essentially what you did. To load that sample object data into R, just use `load()` and the path to the _.rda_ file (`example.data.path` or `"C:/workspace2/mt663.rda"`)
...
#### Thickness based on Diagnostic Features Table
...
hist(f$depth_to_argillic_cm, xlab = 'Depth to argillic top depth (cm)', main = '')
```
...
### Comparing Horizon Data within Diagnostic Features/Intervals
...
The `soilReports` R package is essentially just a collection of R Markdown (_.Rmd_) documents, each with a "manifest" that describes any required dependencies, configuration files or inputs. These _.Rmd_ files can be used to generate HTML, PDF, Word, Markdown documents with a variety of forms, templates and applications.
...
### Exercise: Run the `region11/lab_summary_by_taxonname` Soil Report
Let's demonstrate how to run an existing _.Rmd_ report to summarize laboratory data for a soil series.
...
Queries against the NASIS local database can be written in [T-SQL](https://en.wikipedia.org/wiki/Transact-SQL) which is the dialect of [SQL](https://en.wikipedia.org/wiki/SQL) used to communicate with Microsoft SQL Server that you set up a connection to for the pre-course. 
