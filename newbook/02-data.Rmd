...
Here we inspect the number of ochric epipedons in a small subset of 100 pedons (about 10% of the MT647 set). We will download this from the course website to save you the effort of crafting your selected set for this example.
...
download.file("https://github.com/ncss-tech/stats_for_soil_survey/raw/master/data/book/02/mt647.zip", 
              destfile = example.data.path)

unzip(example.data.path, exdir = example.data.dir, overwrite = TRUE)
```

To load the sample object data into R, just use `load()` and the path to the _.rda_ file (`example.data.path` or `"C:/workspace2/mt663.rda"`)
...
The `aqp` package has several functions that do logic checks on `SoilProfileCollection` objects. The main method is `checkHzDepthLogic` which returns a data.frame containing the results of running four logic tests on the horizon data from each profile.
...
download.file("https://github.com/ncss-tech/stats_for_soil_survey/raw/master/data/book/02/mt663.zip", 
              destfile = example.data.path)
...
hist(mt663$argillic_thickness_cm, xlab = 'Thickness of argillic (cm)', main = '')
hist(mt663$depth_to_argillic_cm, xlab = 'Depth to argillic top depth (cm)', main = '')
```
...
diagnosticPropertyPlot(mt663[1:30,], v, k = 5,
                       grid.label = 'site_id', 
