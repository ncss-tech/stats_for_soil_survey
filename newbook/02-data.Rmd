...
### Logical Operators
  ...
  - `%in%` Equivalent to `IN ()` in SQL; same logic as `match()`
...
#### Important syntax options for REGEX pattern matching

  - `|` is exquivalent to the `OR` operator for multiple expressions: 
...
### Filtering Data by Logical Criteria with `aqp::subset`
...
#### `tidyverts` and the `tsibble` package

[`tidyerts`](https://tidyverts.org/) is all about _time-series_ in the [tidyverse](https://tidyverse.org/). 

The [`tsibble`](https://github.com/tidyverts/tsibble) package provides a data structure for tidy temporal data and models.

### EXERCISE: Prepare Example Data
...
## Comparing Results with `rmHzErrors` 
...
subset(site(mt663), ochric.epipedon)
...
subset(site(mt663err), ochric.epipedon)

# these are the mt663 bad.pedon.ids
get('bad.pedon.ids', envir = soilDB.env)

# none of the bad pedons are in mt663 (rmHzErrors=TRUE)
get('bad.pedon.ids', envir = soilDB.env) %in% mt663$pedon_id

# all of the bad pedons are in mt663err (rmHzErrors=FALSE)
get('bad.pedon.ids', envir = soilDB.env) %in% mt663err$pedon_id

# both good and bad pedons are in mt663err
mt663err$pedon_id %in% get('bad.pedon.ids', envir = soilDB.env)
```
...
### Exercise: run the region11/lab\_summary\_by\_taxonname
