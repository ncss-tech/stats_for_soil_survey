...
All slots in the collection have a relationship to the "main" profile (_i_) index -- so when you remove profiles, all associated records (e.g. diagnostics, horizons) in the object are removed.

Similarly, when all the horizons in a profile are removed (e.g. you request the 6th _j_-index from a profile that has only 5), the entire corresponding profile (site index) is removed.
