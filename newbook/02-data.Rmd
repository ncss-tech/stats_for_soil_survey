...
The [`SoilProfileCollection`](http://ncss-tech.github.io/AQP/aqp/aqp-intro.html) class (SPC) provided by the `aqp` package is a specialized structure for soil data analysis. It simplifies the process of working with collections of data associated with soil profiles, e.g., site-level data, horizon-level data, spatial data, diagnostic horizon data, metadata, etc. 
...
Alternatively, use a circular histogram. Days of the year progress clockwise from 0.
```{r, eval =  FALSE}
library(circular)

d.circ <- circular(d$doy, type = 'angles', units = 'degrees', rotation = 'clock', zero = pi/2)
par(mar = c(0, 0, 0, 0))
plot.circular(d.circ, stack = TRUE, bg = 'royalblue', pch = 21, sep = 0.04, shrink = 1.5)
```
