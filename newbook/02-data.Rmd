...
- Begin to explore the power of regular expression (regex) patterns
...
The `plotSPC()` or `plot()` function applied to a `SoilProfileCollection` object generates sketches based on horizon depths, designations, and colors. The [`SoilProfileCollection` Reference](http://ncss-tech.github.io/aqp/articles/Introduction-to-SoilProfileCollection-Objects.html#plotting-soilprofilecollection-objects) contains many examples demonstrating way in which these sketches can be customized.
...
The [Soil Profile Sketches tutorial](https://ncss-tech.github.io/AQP/aqp/sketches.html) contains additional examples that demonstrate various ways to customize soil profile sketches.  
...
### Further Reading / Reference
   ...
   * The [`SoilProfileCollection` Reference](http://ncss-tech.github.io/aqp/articles/Introduction-to-SoilProfileCollection-Objects.html) contains examples based on the `sp4` sample dataset. This is a good exercise for the class and reference for future work.
   
   * The [Querying Soil Series Data](https://ncss-tech.github.io/AQP/soilDB/soil-series-query-functions.html) document contains a more detailed explanation of how the data behind `fetchOSD` were created and how to interpret its results.

   * The [Competing Soil Series](https://ncss-tech.github.io/AQP/soilDB/competing-series.html) document contains a number of examples related to getting OSD morphology using `fetchOSD`, laboratory data using `fetchKSSL`, and summary via profile sketches and slice-wise aggregation.
   
   * The ["SPC Plotting Ideas"](https://ncss-tech.github.io/AQP/aqp/SPC-plotting-ideas.html) document outlines some advanced techniques for arranging and annotating soil profile sketches.

   * Several examples pertaining to SDA, soil texture, and soil profile sketches are explored in this [Random Walk 001](https://ncss-tech.github.io/AQP/random-walks/walk-001.html).
...
```{r}
as.POSIXct("01/01/1970", format = "%m/%d/%Y")
```
...
## Exercise: Generalized Horizons with Loafercreek 
...
This demonstrates one way of grouping horizon data for determining the Range in Characteristics of layers within a Soil Series or a SSURGO component.
 ...
 - [Generalized Horizon Labels with Loafercreek](http://ncss-tech.github.io/stats_for_soil_survey/chapters/2_data/genhz_homework.html)
...
_Notes about default settings and handling of `NULL` (missing data elements or records)_
...
The column names are the names of variables that you could join to your site or horizon data by various means. Generally these variable names, with a few exceptions, mirror the [NASIS 7 data model names](https://www.nrcs.usda.gov/resources/guides-and-instructions/nasis-741-database-model-metadata-pdf-documents).
...
## Exercise: Diagnostic Horizons in Your Own Data
