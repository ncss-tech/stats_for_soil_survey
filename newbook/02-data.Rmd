...
The base R functions for accessing and setting `data.frame` columns by name such as `$` and `[[` work for `SoilProfileCollection` objects, too. Note that single columns in `data.frame` objects are accessed with `$` and `[[`, while combinations of rows and columns are accessed with `[`.
...
##### Analogues and Extention of Row Access: `[`
